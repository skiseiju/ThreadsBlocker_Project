...
export const UI = {
  ...
  normalizeReportPath: (path) => {
    const source =
      Array.isArray(path) && path.length > 0 ? path : UI.getDefaultReportPath();
  ...
  getHumanErrorMessage: (result, fallback = "未知錯誤") => {
    ...
    const reasons = Array.isArray(result.reasons)
      ...
      : [];
    if (primary && reasons.length > 0 && !primary.includes(reasons[0]))
      return `${primary}: ${reasons.join(", ")}`;
  ...
  isValidPlatformEvent: (event) => {
    ...
    return (
      (eventType === "block" || eventType === "report") &&
      !!accountId &&
  ...
  injectStyles: () => {
    ...
    style.textContent = `
            ...
            .hege-checkbox-container.checked .hege-svg-icon { color: #ff3b30; }
            .hege-checkmark { display: none; stroke: #000; stroke-width: 3; stroke-linecap: round; stroke-linejoin: round; fill: none; }
  ...
  createPanel: (callbacks) => {
    ...
    const htmlContent = `
            ...
            <div class="hege-content">
                ...
                <div class="hege-menu-item" id="hege-export-csv-item">
                    <span>匯出選取為 CSV</span>
                    <span class="status" id="hege-export-csv-count">0 筆</span>
                </div>
    ...
    bindClick("hege-export-csv-item", callbacks.onExportCsv);
    ...
    document.getElementById("hege-header").onclick = (e) => {
      if (e.target.id !== "hege-toggle")
        document.getElementById("hege-toggle").click();
    };
    ...
    document.addEventListener("click", (e) => {
      ...
      if (
        p &&
        !p.classList.contains("minimized") &&
        !p.contains(e.target) &&
        !e.target.closest("#hege-panel")
      ) {
  ...
  anchorPanel: () => {
    ...
    if (anchor) {
      ...
      if (rect.width > 0 && rect.height > 0 && rect.top >= 0) {
        ...
        const headerHeight =
          panel.querySelector("#hege-header")?.offsetHeight || 38;
        const contentHeight =
          panel.querySelector(".hege-content")?.scrollHeight || 360;
        const needsOpenUp =
          rect.top + headerHeight + contentHeight + 12 > window.innerHeight;
        ...
        if (CONFIG.DEBUG_MODE)
          console.log(`[留友封] Menu Anchored at ${rect.top}px`);
      }
    } else {
      ...
      if (
        !panel.style.top ||
        parseInt(panel.style.top) > 200 ||
        parseInt(panel.style.top) < 50
      ) {
  ...
  showPlatformSyncConsentModal: (options = {}) => {
    ...
    overlay.querySelector("#hege-platform-sync-manual").onclick = () =>
      finish(false);
    overlay.querySelector("#hege-platform-sync-enable").onclick = () =>
      finish(true);
  },

  showPlatformManualUploadReminderModal: () => {
    if (document.getElementById("hege-platform-manual-reminder-overlay"))
      return;
    if (Storage.hasPlatformSyncConsentForCurrentVersion()) return;
  ...
  showReportPicker: (callback, options = {}) => {
    ...
    overlay.style.cssText =
      'position:fixed; inset:0; background:rgba(0,0,0,0.72); z-index:2147483647; display:flex; align-items:center; justify-content:center; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;';
    Utils.setHTML(
      overlay,
      `
    ...
    const render = () => {
      ...
      while (node && typeof node === "object") {
        ...
        label.style.cssText =
          "display:flex; flex-direction:column; gap:6px; font-size:12px; color:#aaa;";
        const text = document.createElement("span");
        const select = document.createElement("select");
        select.dataset.level = String(depth);
        select.style.cssText =
          "width:100%; box-sizing:border-box; background:#111; border:1px solid #444; color:#f5f5f5; padding:9px 10px; border-radius:8px; font-size:13px; outline:none;";
        ...
        select.value = options.includes(reportPath[depth])
          ? reportPath[depth]
          : options[0];
        select.onchange = () => {
  ...
  showSettingsModal: (callbacks) => {
    ...
    const showSettingsView = (view) => {
      overlay
        .querySelectorAll("[data-hege-settings-view-panel]")
        .forEach((panel) => {
    ...
    if (speedStatusEl)
      speedStatusEl.textContent =
        speedLabels[Storage.get(CONFIG.KEYS.SPEED_MODE) || "smart"];
    ...
    // Meta 每日安全上限
    const dailyLimitSelect = overlay.querySelector(
      "#hege-s-daily-limit-select",
    );
    if (dailyLimitSelect) {
    ...
    const dailyReportLimitSelect = overlay.querySelector(
      "#hege-s-daily-report-limit-select",
    );
    if (dailyReportLimitSelect) {
      ...
      const dailyReportLimitRow = overlay.querySelector(
        "#hege-s-daily-report-limit",
      );
      if (dailyReportLimitRow)
        dailyReportLimitRow.onclick = (e) => e.stopPropagation();
    }

    const reportVisualDebugToggle = overlay.querySelector(
      "#hege-s-report-visual-debug-toggle",
    );
    if (reportVisualDebugToggle) {
      reportVisualDebugToggle.checked =
        Storage.get(CONFIG.KEYS.REPORT_VISUAL_DEBUG) === "true";
      reportVisualDebugToggle.onchange = (e) => {
        Storage.set(
          CONFIG.KEYS.REPORT_VISUAL_DEBUG,
          e.target.checked ? "true" : "false",
        );
        UI.showToast(
          e.target.checked ? "檢舉可視化已開啟" : "檢舉可視化已關閉",
        );
      };
      const reportVisualDebugRow = overlay.querySelector(
        "#hege-s-report-visual-debug-row",
      );
      if (reportVisualDebugRow)
        reportVisualDebugRow.onclick = (e) => e.stopPropagation();
    }

    const blockVisualDebugToggle = overlay.querySelector(
      "#hege-s-block-visual-debug-toggle",
    );
    if (blockVisualDebugToggle) {
      blockVisualDebugToggle.checked =
        Storage.get(CONFIG.KEYS.BLOCK_VISUAL_DEBUG) === "true";
      blockVisualDebugToggle.onchange = (e) => {
        Storage.set(
          CONFIG.KEYS.BLOCK_VISUAL_DEBUG,
          e.target.checked ? "true" : "false",
        );
        UI.showToast(
          e.target.checked ? "封鎖可視化已開啟" : "封鎖可視化已關閉",
        );
      };
      const blockVisualDebugRow = overlay.querySelector(
        "#hege-s-block-visual-debug-row",
      );
      if (blockVisualDebugRow)
        blockVisualDebugRow.onclick = (e) => e.stopPropagation();
    }

    const autoMarkLeaderToggle = overlay.querySelector(
      "#hege-s-auto-mark-leader",
    );
    if (autoMarkLeaderToggle) {
      autoMarkLeaderToggle.checked =
        Storage.get(CONFIG.KEYS.AUTO_MARK_LEADER) !== "false";
      autoMarkLeaderToggle.onchange = (e) => {
        Storage.set(
          CONFIG.KEYS.AUTO_MARK_LEADER,
          e.target.checked ? "true" : "false",
        );
        UI.showToast(
          e.target.checked
      ...
      const autoMarkLeaderRow = overlay.querySelector(
        "#hege-s-auto-mark-leader-row",
      );
      if (autoMarkLeaderRow)
        autoMarkLeaderRow.onclick = (e) => e.stopPropagation();
    }

    bind("hege-s-sponsor", () => {
      alert(
        "目前還不急著收贊助，但歡迎來看看我還可以幫你解決什麼 → skiseiju.com",
      );
      window.open(
        "https://skiseiju.com?utm_source=extension&utm_medium=popup",
        "_blank",
      );
    });
  },
  ...
  supportsPlatformAutoSync: () => {
    ...
    return (
      clientPlatform === "chrome_extension" ||
      clientPlatform === "firefox_extension"
    );
  },
  ...
  buildPlatformExportPayload: (options = {}) => {
    const platformSyncEnabled = Object.prototype.hasOwnProperty.call(
      options,
      "platformSyncEnabled",
    )
      ? !!options.platformSyncEnabled
      : Storage.hasPlatformSyncConsentForCurrentVersion() &&
        Storage.getPlatformSyncEnabled();
    const platformSyncLastAt = Object.prototype.hasOwnProperty.call(
      options,
      "platformSyncLastAt",
    )
    ...
    const evidenceIndexRaw = Storage.getJSON(
      CONFIG.KEYS.SOURCE_EVIDENCE_INDEX,
      {},
    );
    const evidenceIndexMap =
      evidenceIndexRaw &&
      typeof evidenceIndexRaw === "object" &&
      !Array.isArray(evidenceIndexRaw)
        ? evidenceIndexRaw
        : {};
    ...
    const reportEvents = reportHistory.map((entry, idx) => {
      ...
      return {
        ...
        profileUrl: entry.username
          ? `https://www.threads.com/@${entry.username}`
          : "",
        eventAt: entry.t || 0,
    ...
    unifiedEvents.forEach((event) => {
      ...
      if (event.reportPrimaryCategory)
        agg.reportPrimaryCategories.add(event.reportPrimaryCategory);
      if (event.reportLeafCategory)
        agg.reportLeafCategories.add(event.reportLeafCategory);
      if (event.eventAt > 0) {
        if (!agg.firstSeenAt || event.eventAt < agg.firstSeenAt)
          agg.firstSeenAt = event.eventAt;
        if (!agg.lastSeenAt || event.eventAt > agg.lastSeenAt)
          agg.lastSeenAt = event.eventAt;
      }
    });

    const accounts = Object.values(accountAgg)
      .map((acc) => {
    ...
    unifiedEvents.forEach((event) => {
      ...
      if (event.blockReasonCode) {
        src.blockReasonCounts[event.blockReasonCode] =
          (src.blockReasonCounts[event.blockReasonCode] || 0) + 1;
      }
    ...
    sourceEvidenceList.forEach((ev) => {
      ...
      if (ev.sourceOwner)
        sourceAgg[ev.sourceUrl].sourceOwners.add(ev.sourceOwner);
      if (ev.snippet) sourceAgg[ev.sourceUrl].sourceTextSamples.add(ev.snippet);
    });

    const sources = Object.values(sourceAgg)
      .map((src) => {
        ...
        const manipulationSignalScore = Math.min(
          100,
        ...
        const manipulationRiskLevel =
          manipulationSignalScore >= 65
            ? "high"
            : manipulationSignalScore >= 45
              ? "medium"
              : "low";
        return {
          ...
          topTopicHints: Object.entries(src.topicHintCounts)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 8)
            .map(([topicHint, count]) => ({ topicHint, count })),
    ...
    const narrativeSeeds = sources.slice(0, 20).map((src) => ({
      ...
      dominantReportPaths: Object.entries(src.reportPathCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
        .map(([path, count]) => ({ path, count })),
      dominantBlockReasons: Object.entries(src.blockReasonCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
    ...
    const sourceEvidenceWithSnippetCount = sourceEvidenceList.filter(
      (item) => !!item.snippet,
    ).length;
    const sourcedBlockCount = blockEvents.filter(
      (event) => !!event.sourceUrl,
    ).length;
    const sourcedReported = reportEvents.filter(
      (event) => !!event.sourceUrl,
    ).length;
    const sourceCoveragePct =
      blockEvents.length > 0
        ? Math.round((sourcedBlockCount / blockEvents.length) * 100)
        : 0;
    const reportSourceCoveragePct =
      reportEvents.length > 0
        ? Math.round((sourcedReported / reportEvents.length) * 100)
        : 0;
    const sourceEvidenceCoveragePct =
      sources.length > 0
        ? Math.min(
            100,
            Math.round((sourceEvidenceList.length / sources.length) * 100),
          )
        : 0;
    const sourceEvidenceSnippetCoveragePct =
      sourceEvidenceList.length > 0
        ? Math.round(
            (sourceEvidenceWithSnippetCount / sourceEvidenceList.length) * 100,
          )
        : 0;
    ...
    return {
      ...
      fieldSpec: {
        ...
        accounts: [
          ...
          "sourceOwners",
          "blockReasons",
          "reportPrimaryCategories",
          "reportLeafCategories",
          "platformReview",
        ],
        events: [
          "eventId",
      ...
      summary: {
        ...
        bothBlockedAndReportedAccountCount: accounts.filter(
          (acc) => acc.blockEventCount > 0 && acc.reportEventCount > 0,
        ).length,
  ...
  tryAutoSyncPlatformUpload: async (options = {}) => {
    if (!UI.supportsPlatformAutoSync()) {
      if (
        !Storage.hasPlatformSyncConsentForCurrentVersion() &&
        UI.shouldShowManualUploadReminder()
      ) {
    ...
    if (!Storage.hasPlatformSyncConsentForCurrentVersion()) {
      UI.showPlatformSyncConsentModal({
        onEnable: () =>
          UI.tryAutoSyncPlatformUpload({
            ...options,
            source: options.source || "consent_modal",
          }),
      });
      return { code: 204, skipped: "pending_version_consent" };
    }
    if (!Storage.getPlatformSyncEnabled())
      return { code: 204, skipped: "disabled" };
    ...
    const existingLock =
      parseInt(Storage.get(CONFIG.KEYS.PLATFORM_SYNC_LOCK, "0") || "0", 10) ||
      0;
  ...
  tryRepairPlatformReupload: async (options = {}) => {
    ...
    if (!UI.supportsPlatformAutoSync())
      return { code: 204, skipped: "unsupported_platform" };
    if (Storage.getPlatformReuploadRepairDone())
      return { code: 204, skipped: "repair_already_done" };
    ...
    try {
      ...
      if (
        ...
        (!result.duplicate || result.rawStored)
      ) {
  ...
  showAnalyticsReport: (options = {}) => {
    ...
    const analyticsShowAdvanced =
      Storage.get(CONFIG.KEYS.ANALYTICS_SHOW_ADVANCED, "false") === "true";
    const platformSyncPreference = Storage.getPlatformSyncPreference();
    const hasPlatformSyncConsentForCurrentVersion =
      Storage.hasPlatformSyncConsentForCurrentVersion();
    const platformSyncEnabled =
    ...
    const endlessQueue = Storage.postReservoir
      .getAll()
      .filter((p) => p.advanceOnComplete);
    const reportHistory = Storage.getJSON(CONFIG.KEYS.REPORT_HISTORY, []);
    const evidenceIndexRaw = Storage.getJSON(
      CONFIG.KEYS.SOURCE_EVIDENCE_INDEX,
      {},
    ...
    const cooldownUntil = parseInt(
      Storage.get(CONFIG.KEYS.COOLDOWN) || "0",
      10,
    );
    ...
    const getTs = (u) => {
      const e = ts[u];
      return typeof e === "object" && e !== null ? e.t || 0 : e || 0;
    };
    const getEntry = (u) => {
      const e = ts[u];
      return typeof e === "object" && e !== null ? e : { t: e || 0 };
    ...
    const structuredEntries = knownEntries.filter(
      (e) =>
        e.t > 0 && (e.src || e.reason || e.batch || e.postOwner || e.postText),
    );
    ...
    const blockTimes = db
      ...
      .filter((t) => t > 0)
      .sort((a, b) => a - b);
    const reportTimes = reportHistory
      .map((entry) => entry.t || 0)
      .filter((t) => t > 0)
      .sort((a, b) => a - b);
    ...
    const sourceEvidenceRecent7dCount = sourceEvidenceList.filter(
      (item) => (item.updatedAt || 0) >= now - 7 * 24 * 3600 * 1000,
    ).length;
    const sourceEvidencePreview = sourceEvidenceList.slice(0, 8);
    const earliest =
      actionTimes.length > 0
        ? new Date(actionTimes[0]).toLocaleDateString()
        : "-";
    const latest =
      actionTimes.length > 0
        ? new Date(actionTimes[actionTimes.length - 1]).toLocaleDateString()
        : "-";
    ...
    db.forEach((u) => {
      ...
      if (entry.src) {
        ...
        srcMap[entry.src].reasons[reason] =
          (srcMap[entry.src].reasons[reason] || 0) + 1;
      }
    });
    ...
    const sourceLinkedCount = Object.values(srcMap).reduce(
      (sum, info) => sum + info.count,
      0,
    );
    const sourceCoveragePct =
      totalBlocked > 0
        ? Math.round((sourceLinkedCount / totalBlocked) * 100)
        : 0;
    const topSourceShare =
      totalBlocked > 0 && topSources.length > 0
        ? Math.round((topSources[0][1].count / totalBlocked) * 100)
        : 0;
    const reportSourceMap = {};
    reportHistory.forEach((entry) => {
      const src = entry.sourceUrl || "";
      if (!src) return;
      if (!reportSourceMap[src])
        reportSourceMap[src] = { count: 0, paths: {}, sources: {} };
      reportSourceMap[src].count++;
      const pathLabel = Array.isArray(entry.path) ? entry.path.join(" > ") : "";
      if (pathLabel)
        reportSourceMap[src].paths[pathLabel] =
          (reportSourceMap[src].paths[pathLabel] || 0) + 1;
      const sourceLabel = entry.source || "unknown";
      reportSourceMap[src].sources[sourceLabel] =
        (reportSourceMap[src].sources[sourceLabel] || 0) + 1;
    });
    const reportSourceLinkedCount = Object.values(reportSourceMap).reduce(
      (sum, info) => sum + info.count,
      0,
    );
    const reportSourceCoveragePct =
      reportHistory.length > 0
        ? Math.round((reportSourceLinkedCount / reportHistory.length) * 100)
        : 0;
    const provenanceMap = {};
    Object.entries(srcMap).forEach(([url, info]) => {
      if (!provenanceMap[url])
        provenanceMap[url] = {
          blocked: 0,
          reported: 0,
          postText: info.postText || "",
          postOwner: info.postOwner || "",
          reportPaths: {},
        };
    ...
    Object.entries(reportSourceMap).forEach(([url, info]) => {
      ...
      Object.entries(info.paths).forEach(([path, count]) => {
        provenanceMap[url].reportPaths[path] =
          (provenanceMap[url].reportPaths[path] || 0) + count;
      });
    });
    const allProvenanceSources = Object.entries(provenanceMap).sort(
      (a, b) => b[1].blocked + b[1].reported - (a[1].blocked + a[1].reported),
    );
    const topProvenanceSources = allProvenanceSources.slice(0, 12);
    const sourceEvidenceCoveragePct =
      allProvenanceSources.length > 0
        ? Math.min(
            100,
            Math.round(
              (sourceEvidenceList.length / allProvenanceSources.length) * 100,
            ),
          )
        : 0;
    const sourceEvidenceSnippetCoveragePct =
    ...
    const dayLabel = (t) =>
      new Date(t).toLocaleDateString("zh-TW", {
        month: "numeric",
        day: "numeric",
      });
    const dailyCounts = {};
    ...
    const avgPerActiveDay =
      activeDays30 > 0
        ? Math.round((thirtyDayCount / activeDays30) * 10) / 10
        : 0;
    const peakDay =
      Object.entries(dailyCounts).sort((a, b) => b[1] - a[1])[0] || null;

    // 定點絕戰績
    const totalSweepPosts =
      endlessHistory.length + endlessQueue.filter((p) => p.done).length;
    const totalSweepBlocked =
      endlessHistory.reduce((s, h) => s + (h.totalBlocked || 0), 0) +
      endlessQueue
        .filter((p) => p.done)
        .reduce((s, p) => s + (p.totalBlocked || 0), 0);
    const pendingSweepPosts = endlessQueue.filter(
      (p) => p.advanceOnComplete && p.status === "pending",
    ).length;
    ...
    const sourcedReportCount = reportHistory.filter(
    ...
    structuredEntries.forEach((e) => {
      ...
      if (e.postOwner) {
        if (!ownerMap[e.postOwner])
          ownerMap[e.postOwner] = { count: 0, sources: new Set() };
    ...
    const avgBatchSize =
      batchTotal > 0
        ? Math.round(
            (batchCounts.reduce((s, c) => s + c, 0) / batchTotal) * 10,
          ) / 10
        : 0;
    ...
    const insightRows = [
      sourceCoveragePct < 60
        ? `封鎖來源覆蓋率只有 ${sourceCoveragePct}%，有些封鎖帳號還追不到來源貼文。`
        : `封鎖來源覆蓋率 ${sourceCoveragePct}%，大多數封鎖帳號都能回推到來源貼文。`,
      reportHistory.length > 0
        ? reportSourceCoveragePct < 60
          ? `檢舉來源覆蓋率只有 ${reportSourceCoveragePct}%，檢舉路徑已記錄，但還有部分缺少來源貼文。`
          : `檢舉來源覆蓋率 ${reportSourceCoveragePct}%，檢舉帳號也能穩定回推到來源貼文。`
      ...
      legacyCount > 0
        ? `有 ${legacyCount} 筆舊封鎖資料沒有完整來源欄位，分析時要和新版資料分開看。`
        : "目前封鎖資料都帶有可分析的來源欄位。",
      topSourceShare >= 35
        ? `最大單一來源文章占 ${topSourceShare}%，代表封鎖很集中在少數貼文。`
        : `最大單一來源文章占 ${topSourceShare}%，來源分布目前算平均。`,
      allProvenanceSources.length > 0
        ? `目前已整理出 ${allProvenanceSources.length} 篇來源文章，可直接拿去做平台交叉分析。`
        : "目前還沒有可交叉分析的來源文章。",
    ];
            ...
            <div class="hege-manager-box" style="max-width: 580px; width: 90vw; max-height: 85vh;">
                ...
                <div style="padding:16px;overflow-y:auto;max-height:calc(85vh - 60px);">
                    ...
                    <div style="background:#111;border-radius:10px;padding:12px;border:1px solid #2a2a2a;margin-bottom:12px;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:6px;">
                            ${insightRows
                              ...
                              .map(
                                (line) =>
                                  `<div style="font-size:12px;color:#ccc;line-height:1.5;background:#1a1a1a;border-radius:7px;padding:8px 9px;">${Utils.escapeHTML(line)}</div>`,
                              )
                              .join("")}
                        </div>
                    </div>
                    ...
                    <div style="background:#111;border-radius:10px;padding:12px;border:1px solid #2a2a2a;margin-bottom:12px;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:6px;">
                            ${topProvenanceSources
                              .slice(0, 5)
                              .map(([url, info], i) => {
                                const total =
                                  (info.blocked || 0) + (info.reported || 0);
                                return `
                    ...
                    <div style="background:#111;border-radius:8px;padding:12px;border:1px solid #2a2a2a;margin-bottom:12px;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:6px;">
                            ${topProvenanceSources
                              .map(([url, info], i) => {
                                const pathSummary = Object.entries(
                                  info.reportPaths,
                                )
                                  .sort((a, b) => b[1] - a[1])
                                  .slice(0, 2)
                                ...
                                <div style="display:flex;align-items:flex-start;gap:8px;padding:6px;background:#1a1a1a;border-radius:6px;">
                                    ...
                                    <div style="flex:1;min-width:0;">
                                        ...
                                        <div style="font-size:10px;color:#777;margin-top:2px;">
                                            ${info.postOwner ? `作者 @${Utils.escapeHTML(info.postOwner)} · ` : ""}${Object.entries(
                                              info.reasons,
                                            )
                                              .map(
                                                ([k, c]) =>
                        ...
                        <div style="background:#111;border-radius:8px;padding:12px;border:1px solid #2a2a2a;">
                            ...
                            ${
                              topOwners.length > 0
                                ...
                                : `<div style="font-size:12px;color:#666;">沒有可分析的發文者資料</div>`
                            }
                        </div>
                    </div>
                    ...
                    <div style="background:#111;border-radius:8px;padding:12px;border:1px solid #2a2a2a;margin-bottom:12px;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:6px;">
                            ...
                            : `<div style="font-size:12px;color:#666;">目前還沒有可預覽的匯出資料。</div>`
                        }
                    </div>
    ...
    overlay.querySelector(".hege-manager-close").onclick = () =>
      overlay.remove();
    ...
    if (backBtn) {
      backBtn.onclick = () => {
        ...
        try {
          ...
          console.error("[hege] analytics back failed", e);
        }
      };
    }
    overlay.addEventListener("click", (e) => {
    ...
    const exportBtn = overlay.querySelector(
      "#hege-analytics-export-provenance",
    );
    if (exportBtn) {
      exportBtn.onclick = async (e) => {
        ...
        try {
          ...
          UI.showToast(
            `已複製 v2 JSON（封鎖 ${exportPayload.summary.blockEventCount}、檢舉 ${exportPayload.summary.reportEventCount}）`,
          );
        } catch (err) {
    ...
    const uploadStatusEl = overlay.querySelector(
      "#hege-analytics-upload-status",
    );
    const uploadCardEl = overlay.querySelector("#hege-analytics-upload-card");
    const autoSyncToggle = overlay.querySelector(
      "#hege-analytics-auto-sync-toggle",
    );
    if (autoSyncToggle) {
    ...
    if (uploadBtn) {
      uploadBtn.onclick = async (e) => {
        ...
        try {
          exportPayload.syncPreferences.autoSyncEnabled =
            Storage.hasPlatformSyncConsentForCurrentVersion() &&
            Storage.getPlatformSyncEnabled();
          exportPayload.syncPreferences.lastSyncedAt =
            Storage.getPlatformSyncLastAt();
          const result = await Reporter.submitPlatformPayload(exportPayload, {
          ...
          if (Number(result?.code) === 200) {
            if (result?.duplicate) {
              if (uploadStatusEl)
                uploadStatusEl.textContent = `已存在相同批次（ID ${result?.id || "-"}）`;
              UI.showToast(`平台已存在同批資料（ID ${result?.id || "-"}）`);
            } else {
              if (uploadStatusEl)
                uploadStatusEl.textContent = `上傳成功（批次 ID ${result?.id || "-"}；樣本層級 ${result?.trustTier || "legacy"}）`;
    ...
    if (options.focusUpload && uploadCardEl) {
      ...
      uploadCardEl.style.boxShadow =
        "0 0 0 1px rgba(48,209,88,0.45), 0 0 20px rgba(48,209,88,0.2)";
      uploadCardEl.scrollIntoView({ behavior: "smooth", block: "center" });
      if (uploadStatusEl && !uploadStatusEl.textContent) {
        uploadStatusEl.textContent =
          "提示：按「一鍵上傳平台」即可提交一般使用者資料。";
      }
    }
    const toggleAdvancedBtn = overlay.querySelector(
      "#hege-analytics-toggle-advanced",
    );
    if (toggleAdvancedBtn) {
      toggleAdvancedBtn.onclick = (e) => {
        e.stopPropagation();
        Storage.set(
          CONFIG.KEYS.ANALYTICS_SHOW_ADVANCED,
          analyticsShowAdvanced ? "false" : "true",
        );
        overlay.remove();
  ...
  showPostReservoir: (options = {}) => {
    ...
    const modeBadges = (entry) => {
      const baseStyle =
        "display:inline-flex;align-items:center;justify-content:center;gap:4px;min-height:30px;padding:6px 10px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;line-height:1;user-select:none;";
      ...
      return (
        (entry.advanceOnComplete ? onAdvance : offAdvance) +
        " " +
        (entry.longTermLoop ? onLoop : offLoop)
      );
    };
    ...
    const renderOverlay = () => {
      ...
      const pendingAdvanceCount = entries.filter(
        (p) =>
          p.advanceOnComplete &&
          (p.status === "pending" || p.status === "done" || p.done === true),
      ).length;
      ...
      const canStart =
        pendingAdvanceCount > 0 && !isEndlessRunning && hasStartHandler;
      ...
      const resetRunBtn = overlay.querySelector(
        "#hege-reservoir-reset-run-state",
      );
      if (resetRunBtn) {
        resetRunBtn.onclick = () => {
      ...
      overlay.querySelectorAll(".hege-mode-badge").forEach((badge) => {
        badge.onclick = (e) => {
          ...
          if (flag === "advance") {
            ...
            Storage.postReservoir.setFlags(url, {
  ...
  showCockroachManager: (cockroachDb, onRemove, onBack = null) => {
    ...
    const getUname = (c) =>
      typeof c === "string" ? c : c && c.username ? c.username : "";
    const getTime = (c) =>
      c && typeof c === "object" && c.timestamp ? c.timestamp : 0;
    ...
    const renderList = (filter = "") => {
      ...
      if (filtered.length === 0) {
        Utils.setHTML(
          listEl,
          `<div style="padding: 40px; text-align: center; color: #555;">${rawList.length === 0 ? "尚無蟑螂記錄" : "無符合結果"}</div>`,
        );
        return;
      }
      ...
      Utils.setHTML(
        ...
        filtered
          .map((c) => {
            ...
            const timeStr = getTime(c)
              ? new Date(getTime(c)).toLocaleString()
              : "無記錄時間";
      ...
      listEl.querySelectorAll(".hege-cockroach-open-profile").forEach((btn) => {
        btn.onclick = (e) => {
          ...
          UI.showToast(
            `已開啟 @${username} 主頁，可在新 tab 手動定點絕想清的貼文`,
          );
        };
      });
      listEl
        .querySelectorAll(".hege-cockroach-profile-link")
        .forEach((link) => {
  ...
  showBlockManager: (blockedList, timestamps, onUnblock) => {
    ...
    const sortUsers = () => {
      if (sortMode === 0) {
        ...
        // Position Desc (Newest at end of array)
        users = [...blockedList].reverse();
      } else if (sortMode === 3) {
        // Position Asc (Oldest at start of array)
        users = [...blockedList];
      }
    };
    ...
    const renderList = (filter = "") => {
      ...
      if (filtered.length === 0) {
        Utils.setHTML(
          listEl,
          '<div style="padding: 40px; text-align: center; color: #555;">無符合結果</div>',
        );
        return;
      }
      ...
      Utils.setHTML(
        ...
        filtered
          .map((u) => {
            ...
            const time = getTs(u)
              ? new Date(getTs(u)).toLocaleString()
              : "無記錄時間";
    ...
    overlay.querySelector(".hege-manager-close").onclick = () =>
      overlay.remove();
    overlay.querySelector("#hege-manager-cancel").onclick = () =>
      overlay.remove();
