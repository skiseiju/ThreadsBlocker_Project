...
(function () {
  ...
  function migratePostReservoirPhase2() {
    if (Storage.get(CONFIG.KEYS.RESERVOIR_PHASE2_MIGRATED) === "true") {
      console.log(
        "[留友封] Post Reservoir Phase 2 migration already completed; skipping",
      );
      return;
    }
    ...
    const normalizeStatus = (status, done) => {
      if (done) return "done";
      if (status === "error" || status === "done" || status === "cooldown")
        return status;
      return "pending";
    };
    const mergeEntry = (byUrl, entry, source) => {
      ...
      existing.addedAt = Math.min(
        existing.addedAt || Date.now(),
        entry.addedAt || Date.now(),
      );
      existing.lastSweptAt = Math.max(
        existing.lastSweptAt || 0,
        entry.lastSweptAt || 0,
      );
      existing.sweepCount =
        (existing.sweepCount || 0) + (entry.sweepCount || 0);
      existing.batchCount =
        (existing.batchCount || 0) + (entry.batchCount || 0);
      existing.totalBlocked =
        (existing.totalBlocked || 0) + (entry.totalBlocked || 0);

      if (source === "post") {
        const alreadyCanonical =
          Object.prototype.hasOwnProperty.call(entry, "advanceOnComplete") ||
          Object.prototype.hasOwnProperty.call(entry, "longTermLoop");
        existing.longTermLoop = alreadyCanonical ? !!entry.longTermLoop : true;
        existing.advanceOnComplete = alreadyCanonical
          ? !!entry.advanceOnComplete
          : !!existing.advanceOnComplete;
        ...
        existing.status = entry.done
          ? "done"
          : normalizeStatus(existing.status, false);
      }
    ...
    if (localStorage.getItem(CONFIG.KEYS.POST_QUEUE_BACKUP_PHASE2) === null) {
      localStorage.setItem(
        CONFIG.KEYS.POST_QUEUE_BACKUP_PHASE2,
        postRaw || "[]",
      );
    }
  ...
  if (Storage.get(CONFIG.KEYS.VERSION_CHECK) !== CONFIG.VERSION) {
    ...
    const migratedBgQueue = [
      ...new Set([...legacyBgQueue, ...legacyDelayedQueue]),
    ];
    ...
    const preservedReportContext = Storage.getJSON(
      CONFIG.KEYS.REPORT_CONTEXT,
      {},
    );
    const preservedReportBatchUsers = Storage.getJSON(
      CONFIG.KEYS.REPORT_BATCH_USERS,
      [],
    );
    const preservedReportCompletedUsers = Storage.getJSON(
      CONFIG.KEYS.REPORT_COMPLETED_USERS,
      [],
    );
    const shouldPreserveLiveReportState =
      preservedWorkerMode === "report" ||
    ...
    Storage.setJSON(
      ...
      shouldPreserveLiveReportState ? preservedReportQueue : [],
    );
    Storage.setJSON(
      CONFIG.KEYS.REPORT_CONTEXT,
      shouldPreserveLiveReportState ? preservedReportContext : {},
    );
    Storage.setJSON(
      CONFIG.KEYS.REPORT_BATCH_USERS,
      shouldPreserveLiveReportState ? preservedReportBatchUsers : [],
    );
    Storage.setJSON(
      CONFIG.KEYS.REPORT_COMPLETED_USERS,
      shouldPreserveLiveReportState ? preservedReportCompletedUsers : [],
    );
    ...
    try {
      ...
      const resetQ = postQ.map((p) => {
        ...
        if (
          next.longTermLoop === true &&
          next.status === "pending" &&
          (next.lastSweptAt || 0) > 0
        ) {
          next.status = "done";
          next.done = true;
          repaired++;
      ...
      console.log(
        `[留友封] POST_QUEUE repair complete: ${repaired}/${resetQ.length} entries adjusted`,
      );
    } catch (e) {
    ...
    const sweepSessionKeys = [
      ...
      "hege_sweep_auto_triggered_once",
      "hege_sweep_wait_started_at",
      "hege_sweep_lock",
    ...
    Object.keys(localStorage)
      .filter((k) => k.startsWith("hege_sweep_"))
    ...
    console.log(
      `[留友封] Updated to v${CONFIG.VERSION}. Cleared temporary queues${shouldPreserveLiveReportState ? " (preserved live report queue)" : " and report queue/context"}. Migrated delayed queue: ${legacyDelayedQueue.length}`,
    );
  }

  // Clear stale sweep standby flag from previous session
  if (
    !sessionStorage.getItem("hege_sweep_state") &&
    localStorage.getItem("hege_sweep_worker_standby") === "true"
  ) {
  ...
  const forceClear = new URLSearchParams(window.location.search).get(
    "hege_clear",
  );
  if (forceClear === "true") {
  ...
  const isBgPage =
    new URLSearchParams(window.location.search).get("hege_bg") === "true";
  ...
  function main() {
    if (isBgPage) {
      ...
      const sweepRuntime = Utils.getSweepRuntimeState
        ? Utils.getSweepRuntimeState()
        : { running: false };
      const targetEntry = targetCleanUrl
        ? Storage.postReservoir.getByUrl(targetCleanUrl)
        : null;
      const targetIsSweeping =
        !!targetEntry && targetEntry.status === "sweeping";
      const targetSweepingStale =
        targetIsSweeping &&
        Date.now() - (targetEntry.lastSweptAt || 0) > 5 * 60 * 1000;
      if (sweepState === "WAIT_FOR_BG" && targetCleanUrl === currentCleanUrl) {
        if (CONFIG.DEBUG_MODE) {
          console.log(
            "[SweepDriver][RESUME-CHECK]",
            JSON.stringify({
              sweepState,
              currentCleanUrl,
              targetCleanUrl,
        ...
        if (targetSweepingStale) {
          ...
          Core.SweepDriver.updateEntry(targetCleanUrl, (p) => ({
            ...p,
            status: "pending",
            done: false,
          }));
          ...
          console.log(
            "[SweepDriver] Cleared stale WAIT_FOR_BG + sweeping state on startup",
          );
        } else {
          const canResume =
            targetIsSweeping &&
          ...
          if (canResume) {
            if (CONFIG.DEBUG_MODE)
              console.log(
                "[SweepDriver][RESUME-CHECK] resume waitForWorkerDrain",
              );
            Core.SweepDriver.waitForWorkerDrain();
          } else {
            if (CONFIG.DEBUG_MODE)
              console.log(
                "[SweepDriver][RESUME-CHECK] clear transient state (cannot resume)",
              );
            Core.SweepDriver.clearTransientState();
          }
        }
        ...
        sweepState &&
        targetCleanUrl &&
        targetCleanUrl !== currentCleanUrl
      ) {
        if (CONFIG.DEBUG_MODE)
          console.log(
            "[SweepDriver][RESUME-CHECK] clear transient state (target mismatch)",
          );
        Core.SweepDriver.clearTransientState();
      } else if (sweepState && !targetCleanUrl) {
        if (CONFIG.DEBUG_MODE)
          console.log(
            "[SweepDriver][RESUME-CHECK] clear transient state (missing target)",
          );
        Core.SweepDriver.clearTransientState();
      }
      ...
      setTimeout(() => {
        ...
        const toRemind = cockroachDB.filter(
          (c) => now - c.timestamp >= tenDaysMs,
        );
        ...
        if (cooldownUntil > Date.now()) {
          ...
          UI.showConfirm(
            ...
            () => {
              ...
              const cooldownQueue = Storage.getJSON(
              ...
              const merged = [
                ...new Set([...currentQueue, ...cooldownQueue, ...pendingArr]),
              ];
      ...
      const callbacks = {
        ...
        onStartReport: () => {
          ...
          const startWorker = () => {
            ...
            Storage.remove(CONFIG.KEYS.WORKER_STATS);
            Core.startReportDebugBatch({
              trigger: Utils.isMobile() ? "same_tab" : "popup_worker",
            });
            if (Utils.isMobile()) {
          ...
          const enqueueAndStart = (keepBlockSelection) => {
            UI.showReportPicker((path) => {
              ...
              Storage.remove(CONFIG.KEYS.REPORT_RESTORE_PENDING);
              Storage.set(
                CONFIG.KEYS.REPORT_KEEP_BLOCK_SELECTION,
                keepBlockSelection ? "true" : "false",
              );
              Storage.setJSON(CONFIG.KEYS.REPORT_BATCH_PATH, path);
              let added = 0;
              pending.forEach((u) => {
                const sourceUrl =
                  Storage.getJSON(CONFIG.KEYS.REPORT_CONTEXT, {})[u]
                    ?.sourceUrl || Core.findSourcePostUrl(document.body);
                ...
                if (Storage.queueAddUnique(CONFIG.KEYS.REPORT_QUEUE, u))
                  added++;
              });
              const total = Storage.getJSON(
                CONFIG.KEYS.REPORT_QUEUE,
                [],
              ).length;
              Storage.setJSON(
                CONFIG.KEYS.REPORT_BATCH_USERS,
                Storage.getJSON(CONFIG.KEYS.REPORT_QUEUE, []),
              );
              if (pending.length > 0) {
        ...
        onExportCsv: () => {
          // 純匯出當前選取名單為 CSV；不觸發任何 block/report worker
          const pending = Array.from(Core.pendingUsers || new Set());
          if (pending.length === 0) {
            UI.showToast("請先勾選帳號再匯出");
            return;
          }
          ...
          const blockDb = new Set(Storage.getBlockDB());
          const tsMap = Storage.getJSON(CONFIG.KEYS.DB_TIMESTAMPS, {});
          const blockContextMap = Storage.getJSON(
            CONFIG.KEYS.BLOCK_CONTEXT_MAP,
            {},
          );
          ...
          const rows = pending.map((u) => {
            const entry =
              ...
              typeof tsMap === "object" &&
              tsMap[u] &&
              typeof tsMap[u] === "object"
                ? tsMap[u]
                : null;
            const ctx =
              blockContextMap && blockContextMap[u] ? blockContextMap[u] : null;
            const sourceUrl =
              (entry && entry.src) ||
              (ctx && ctx.src) ||
              (ctx && ctx.sourceUrl) ||
              "";
            const sourceText =
              (entry && entry.postText) || (ctx && ctx.postText) || "";
            const sourceOwner =
              (entry && entry.postOwner) || (ctx && ctx.postOwner) || "";
            const reason =
              (entry && entry.reason) || (ctx && ctx.reason) || "manual";
            const batch = (entry && entry.batch) || (ctx && ctx.batch) || "";
            const ts = (entry && entry.t) || (ctx && ctx.updatedAt) || 0;
            return [
              u,
              blockDb.has(u) ? "1" : "0",
              sourceUrl,
          ...
          a.href = url;
          a.download = `hegeblocker-selected-${new Date().toISOString().slice(0, 10)}.csv`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          setTimeout(() => URL.revokeObjectURL(url), 1000);
          UI.showToast(`已匯出 ${pending.length} 筆選取名單為 CSV`);
        },
        onClearSel: () => {
          UI.showConfirm(
            "確定要清除目前的「選取清單」與所有「背景排隊」的帳號嗎？\n(這不會影響已完成的封鎖歷史紀錄)",
            () => {
              Core.pendingUsers.clear();
              Storage.setSessionJSON(CONFIG.KEYS.PENDING, []);
              ...
              Core.blockQueue.forEach((b) => {
                const cb = b.parentElement.querySelector(
        ...
        onSettings: () => {
          const openSettings = (initialView = "home") => {
            UI.showSettingsModal({
              ...
              onExportReportDebug: Utils.isBetaBuild()
                ? () => Core.exportLastReportDebug()
                : null,
              onClearDB: confirmClearDB,
              onCockroach: () =>
                Core.openCockroachManager(() => openSettings()),
              onReservoir: () =>
                UI.showPostReservoir({
                  onStart: () => Core.SweepDriver.startNow(),
                }),
              ...
              onAnalytics: () =>
                UI.showAnalyticsReport({ onBack: () => openSettings("data") }),
            });
          };
          openSettings();
        },
      ...
      const syncCompletedReportSelection = () => {
        const completedUsers = Storage.getJSON(
          CONFIG.KEYS.REPORT_COMPLETED_USERS,
          [],
        );
        if (completedUsers.length === 0) return;
        const keepBlockSelection =
          Storage.get(CONFIG.KEYS.REPORT_KEEP_BLOCK_SELECTION, "true") !==
          "false";
      ...
      setInterval(() => {
        if (!document.getElementById("hege-panel")) {
          console.warn(
            "[留友封] Panel missing from DOM! Attempting re-inject?",
          );
        }
        UI.anchorPanel();
      }, 1500);
      ...
      setInterval(() => {
        if (sessionStorage.getItem("hege_skip_sweep_tick_once") === "true") {
          ...
          if (CONFIG.DEBUG_MODE)
            console.log(
              "[SweepDriver] Skip first auto tick after version update",
            );
          return;
        }
      ...
      if (CONFIG.DEBUG_MODE) {
        window.HegeDebug = {
          forceWakeup: () => {
            ...
            console.log(
              "[DeepSweep-Q] 測試後門觸發：已將所有深層清理貼文的冷卻時間歸零！",
            );
            Core.SweepDriver.tick();
          },
        };
      ...
      setTimeout(async () => {
        try {
          ...
          if (CONFIG.DEBUG_MODE && Number(result?.code) === 200) {
            console.log(
              "[留友封][PlatformSync] upload success:",
              result?.id || "-",
            );
          }
        } catch (err) {
      ...
      if (
        params.get("hege_sweep") === "true" ||
        params.get("hege_post_sweep") === "true"
      ) {
        ...
        const shouldResumeSweepPage =
          !!runtime.running || (entry && entry.status === "sweeping");
        if (shouldResumeSweepPage) {
          Utils.pollUntil(
            () =>
              document.querySelector(
                'a[role="link"], span[role="link"], div[role="button"]',
              ),
            10000,
          ...
          if (CONFIG.DEBUG_MODE)
            console.log(
              "[SweepDriver] Ignore stale hege_sweep param without active runtime",
            );
  ...
  if (CONFIG.DEBUG_MODE) {
    ...
    window.__DEBUG_GENERATE_COCKROACH = (
      username = "test_roach_" + Math.floor(Math.random() * 1000),
    ) => {
      const db = JSON.parse(
        localStorage.getItem(CONFIG.KEYS.COCKROACH_DB) || "[]",
      );
      ...
      console.log(
        `[DEBUG] 已注入大蟑螂 @${username} (時標為 11 天前)。重新裝載網頁後將會觸發回望提醒！`,
      );
    };
  }
})();
