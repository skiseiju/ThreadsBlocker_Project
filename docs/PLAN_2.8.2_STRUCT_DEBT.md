...
| 17 | 2.8.3-beta4 | 回報（使用者實測 2026-08-05） | 三無掃描吐 `followers_dialog_not_found` 後整趟結束；失敗原因不外顯、無診斷、無重試 | 中 | 未動工（任務書已備妥 `.ai/codex_task_283beta4.md`，未派工） |
...
| 20 | 2.8.3-beta4 | beta1 診斷實測抓到（2026-08-05） | `panel/clamp` 仍每 1.5 秒寫一筆內容相同的紀錄，beta1 的穩態抑制沒覆蓋到這個呼叫點 | 小 | 未動工（任務書已備妥 `.ai/codex_task_283beta4.md`，未派工） |
