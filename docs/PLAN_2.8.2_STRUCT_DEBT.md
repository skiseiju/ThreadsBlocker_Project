...
| 編號 | 版本 | 來源 | 一句話 | 規模 | 狀態 |
|---:|---|---|---|---|---|
| 34 | 未定 | code review 2026-08-14（beta28-30 範圍，對抗性驗證） | 封鎖假成功家族：(a) beta29 關閉偵測只盯確認框元素且判定先於限流檢查，確認框被限流視窗換裝時誤判成功、限流斷路器被推遲（`worker.js:2978`）；(b) beta28 複驗把「頁上零 dialog」單獨當成功證據，且 3 秒複驗後未再查限流（`worker.js:3041`） | 中 | 未動工。兩處同屬假成功寫入 BlockDB；(a) 是對舊行為的真回歸、修法便宜（check 內先跑 checkForError），優先；(b) 是 ADR 0026 明文取捨的殘留缺口（複驗後補查限流、或要求 confirmDialog 確實已卸載） |
| 35 | 未定 | code review 2026-08-14 | 失敗名單重新驗證強韌性三缺口：(a) 啟動護欄對殘留鍵只查存在、無 TTL，worker 中途關閉後「重新驗證」被永久擋下且停止鈕清不掉（`core.js:1311`）；(b) 導頁重試界限是 30 秒 TTL 而非次數，背景節流下同一已刪除帳號無限重載（`worker.js:1556`）；(c) runStep 通用封鎖路徑不檢查 WORKER_MODE，唯讀模式在 store 不一致時會落入真實封鎖（`worker.js:1876`，防禦深度） | 中 | 未動工（beta28 新功能後續硬化；(a) 使用者可見、優先） |
