...
> 目前公開版 **v2.8.3**，所有 `2.8.4-betaN` 的修正都標 **待發**，回報者拿不到。
...
| 14 | 2.8.4-beta30 | **待發** | SSOT #12 | 48 小時保留窗硬寫 4 處 | 小 | **Fixed**（beta30；常數收斂到 config.js 單一定義，storage 兩個 ring 改引用，`grep "48 \* 60"` 歸零；`report-debug-context.js` 獨立 TTL 依計畫未合併） |
...
| 24 | 2.8.4-beta1 | **待發** | beta2 實測抓到（2026-08-05） | 等滿 12 秒仍找不到 profile root，頁面真的沒 render | 中 | **Fixed**（`42651a4`；逾時後重載一次再試，並記錄救回率待實機驗證） |
| 25 | 2.8.4-beta30 | **待發** | **beta4 實測抓到**（2026-08-05） | 噪音轉移到 `panel/reposition` 與 `selection/layout`：簽章含會變動的計數欄位，去重形同虛設 | 小 | **Fixed**（實為簽章 SSOT 重構時已修：漂移計數全在 `DIAGNOSTIC_SIGNATURE_MEASUREMENT_FIELDS`，簽章只投影狀態欄位。beta30 盤點全部簽章呼叫點確認無漏網，補回歸測試釘住；閒置 60 秒驗收待下次實機診斷順帶看） |
| 26 | 2.8.4-beta2 | **待發** | 回報 #47 第二句 | 回報問題視窗訊息太長，送出按鈕被推到畫面外 | 小 | **Fixed**（`dc30726`；外框限高、內容區捲動、送出列 sticky） |
| 28 | 2.8.4-beta3 | **待發** | Codex 複核 2.8.4-beta1（2026-08-08） | profile root 重試標記沒有時效檢查，worker 中途關閉後標記永久殘留，同一帳號下次直接被當成已重試而不給重載 | 小 | **Fixed**（2.8.4-beta3；加入 30 秒時效與無效標記清除） |
| 29 | 2.8.4-beta3 | **待發** | Codex 複核 2.8.4-beta1（2026-08-08） | profile root 等待被速度模式倍率縮放，加速模式只等 4.8 秒。**2.8.3 起就存在，影響所有加速模式使用者** | 中 | **Fixed**（2.8.4-beta3；profile root 明確走不縮放等待） |
| 31 | 2.8.4-beta3 | **待發** | Codex 複核 2.8.4-beta1（2026-08-08） | 重載發出到新頁接手的空窗期，停止指令無法取消已發出的 reload | 小 | **Fixed**（2.8.4-beta3；重載前與初始化優先檢查停止） |
| 32 | 2.8.4-beta5 | **待發** | Codex 診斷複核（2026-08-10） | `renderTriggered` 在 reload 呼叫前只依 capability 寫成 true，若呼叫失敗或新頁未接手，診斷會誤稱已觸發 | 小 | **Fixed**（2.8.4-beta5；拆成要求／恢復，另記 unavailable／call failed） |
| 45 | 2.8.4-beta35 | **待發** | 使用者實機回報（2026-08-15，繞五版才定位） | 個人頁往下滑，貼文的勾選框與命名標籤整排消失。根因是用「按鈕在畫面上緣以內」判斷標題區，捲動後貼文 top 變小甚至為負，該條件對負數永遠成立，於是每則捲過頂端的貼文都被判成標題區、蓋永久跳過印記；另一處對稱邏輯還會移除已注入的勾選框 | 中 | **Fixed**（beta35；兩處座標判定移除，改由結構式的 isInlinePostElementContext 判定，新增迴歸測試釘住。實測門檻 330，top=-370/-204/272 三則全漏、380/856 兩則正常，零例外。2026-08-15 使用者實機確認正常）。**教訓**：症狀是「捲動後消失」，第一輪就該懷疑座標判準；前四輪（beta32/33/34 兩輪）修的洞都真實存在但非本症狀成因 |
| 46 | 2.8.4-beta37 | **待發** | 使用者實機回報（2026-08-15） | 「命名可疑」誤標真人：`sweetrice039220412`（顯示名「吳佳玲」，2.1 萬粉絲）與網軍帳號形狀相同，beta33 為了讓貼文卡片也能標記而採用帳號名形狀初判，形狀本身無法區分真人 | 小 | **Fixed**（beta37；形狀初判收回，改走「掃描已持久化旗標」與「個人頁標題取顯示名雙重比對」兩條有依據的路。順帶修正待審清單標籤強度分流：命名命中但非三無者不再標成「疑似假帳號」。2026-08-15 使用者實機確認正常） |
| 34 | 2.8.4-beta31 | **待發** | code review 2026-08-14（beta28-30 範圍，對抗性驗證） | 封鎖假成功家族：(a) beta29 關閉偵測只盯確認框元素且判定先於限流檢查，確認框被限流視窗換裝時誤判成功、限流斷路器被推遲（`worker.js:2978`）；(b) beta28 複驗把「頁上零 dialog」單獨當成功證據，且 3 秒複驗後未再查限流（`worker.js:3041`） | 中 | **Fixed**（beta31；(a) check 先查限流再判移除，(b) 複驗後補查限流＋零 dialog 需確認框確實卸載，診斷加 confirmDialogDetached；換裝時序有測試釘住） |
| 39 | 2.8.4-beta32 | **待發** | 使用者提供實機樣本（2026-08-14，四筆） | 網軍命名結構換代：顯示名去空白小寫恰為帳號名前綴，後接 2-4 字母亂碼與 5 碼數字（`Kenneth Berry` / `kennethberryaei31413`）。四筆全有盜用頭像，動物字典與 noAvatar 門檻都抓不到 | 中 | **Fixed**（beta32，ADR 0028；顯示名鏡像雙重吻合判定＋與拼音對稱進 isThreeNo，有活動者掛琥珀標籤。兩段式求值避免逐列 DOM 成本） |
