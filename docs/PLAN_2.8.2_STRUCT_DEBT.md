...
| 16 | 2.8.3-beta3 | 回報（使用者實測 2026-08-05） | 三無掃描結束後 worker 停在白畫面很久才跳出勾選畫面，期間無任何訊息 | 中 | **取證已落地**（`0a0d507`；該區間原本觀測點為 0，現有 10 個）；成因待實機 ring |
...
| 8 | 2.8.3-beta10 | SSOT #1 | 「是否執行中」判斷 13 處分叉，stopping 被當成沒在跑 | 大 | **命名輪已落地**（`e411a91`；13 處改呼叫具名函式，行為零變化，80 格真值表對舊寫法全等）；**收斂待決**，見下方第 7 節 |
...
| 5 | 2.8.3-beta3 | 遺留 | 檢舉偶發 `submit_not_confirmed`，實為第一次 action 後下一步沒出現 | 中 | **Fixed**（`0a0d507`；逾時改記 `missing_report_step`，該區間補上觀測點） |
| 14 | 2.8.4-beta30 | SSOT #12 | 48 小時保留窗硬寫 4 處 | 小 | **Fixed**（beta30；常數收斂到 config.js 單一定義，storage 兩個 ring 改引用，`grep "48 \* 60"` 歸零；`report-debug-context.js` 獨立 TTL 依計畫未合併） |
...
| 25 | 2.8.4-beta30 | **beta4 實測抓到**（2026-08-05） | 噪音轉移到 `panel/reposition` 與 `selection/layout`：簽章含會變動的計數欄位，去重形同虛設 | 小 | **Fixed**（實為簽章 SSOT 重構時已修：漂移計數全在 `DIAGNOSTIC_SIGNATURE_MEASUREMENT_FIELDS`，簽章只投影狀態欄位。beta30 盤點全部簽章呼叫點確認無漏網，補回歸測試釘住；閒置 60 秒驗收待下次實機診斷順帶看） |
...
| 45 | 2.8.4-beta35 | 使用者實機回報（2026-08-15，繞五版才定位） | 個人頁往下滑，貼文的勾選框與命名標籤整排消失。根因是用「按鈕在畫面上緣以內」判斷標題區，捲動後貼文 top 變小甚至為負，該條件對負數永遠成立，於是每則捲過頂端的貼文都被判成標題區、蓋永久跳過印記；另一處對稱邏輯還會移除已注入的勾選框 | 中 | **Fixed**（beta35；兩處座標判定移除，改由結構式的 isInlinePostElementContext 判定，新增迴歸測試釘住。實測門檻 330，top=-370/-204/272 三則全漏、380/856 兩則正常，零例外。2026-08-15 使用者實機確認正常）。**教訓**：症狀是「捲動後消失」，第一輪就該懷疑座標判準；前四輪（beta32/33/34 兩輪）修的洞都真實存在但非本症狀成因 |
| 46 | 2.8.4-beta37 | 使用者實機回報（2026-08-15） | 「命名可疑」誤標真人：`sweetrice039220412`（顯示名「吳佳玲」，2.1 萬粉絲）與網軍帳號形狀相同，beta33 為了讓貼文卡片也能標記而採用帳號名形狀初判，形狀本身無法區分真人 | 小 | **Fixed**（beta37；形狀初判收回，改走「掃描已持久化旗標」與「個人頁標題取顯示名雙重比對」兩條有依據的路。順帶修正待審清單標籤強度分流：命名命中但非三無者不再標成「疑似假帳號」。2026-08-15 使用者實機確認正常） |
