...
| 15 | 2.8.3-beta1 | 查證 | 正式版輕量診斷查不到根因：面板每 1.5 秒固定寫 2 筆洗掉 ring，且輕量層用 `slice(-120)` 位置切法丟掉 priority 保護過的關鍵筆 | 中 | **已落地待實機測**（`5d9493d`，兩輪；Fable 複核補上四層分級與通用頻率上限；全套 80 檔非沙箱環境全過） |
