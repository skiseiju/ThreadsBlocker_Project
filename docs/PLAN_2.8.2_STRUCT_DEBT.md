...
列序即分區（2026-08-14 重整）：**開放真 bug → 取證等資料 → 結構債 → 已修歸檔**。試算表「工作項」分頁鏡像同一順序。

> **「狀態」與「上線」是兩件事。** 狀態＝程式改好了沒；上線＝使用者手上有沒有。
> 目前公開版 **v2.8.4**（2026-08-16，CWS 已送審、AMO 待簽章、GitHub 已發）。原 12 項「待發」已全數上線，改標 v2.8.4。
> 實例：第 26 項（回報視窗沒捲軸）2026-08-07 就修好，但只在未發布的 beta2，
> 使用者 2026-08-15 仍以線上回報 #55 回報同一件事。盤點「線上還有哪些痛點活著」
> 時要看**上線**欄，不能只看狀態欄的 Fixed。

| 編號 | 版本 | 上線 | 來源 | 一句話 | 規模 | 狀態 |
|---:|---|:-:|---|---|---|---|
| 35 | 未定 | — | code review 2026-08-14 | 失敗名單重新驗證強韌性三缺口：(a) 啟動護欄對殘留鍵只查存在、無 TTL，worker 中途關閉後「重新驗證」被永久擋下且停止鈕清不掉（`core.js:1311`）；(b) 導頁重試界限是 30 秒 TTL 而非次數，背景節流下同一已刪除帳號無限重載（`worker.js:1556`）；(c) runStep 通用封鎖路徑不檢查 WORKER_MODE，唯讀模式在 store 不一致時會落入真實封鎖（`worker.js:1876`，防禦深度） | 中 | **(a) Fixed**（beta31；護欄改「worker 非忙碌即視殘留為孤兒」自動清除後啟動，忙碌仍擋）；(b)(c) 未動工 |
| 36 | 未定 | — | code review 2026-08-14 | beta29 observer 效能與偵測缺口：check() 無節流、每批 mutation 強制同步 layout，同分頁模式明顯卡頓；只聽 childList，attribute/style 方式關閉偵測不到；失敗路徑固定 15+3 秒比舊版 5 秒慢（`utils.js:264`）。另 `observer: true` 診斷欄位不在白名單被靜默丟棄，與註解宣稱不符（`worker.js:2990`） | 中 | 未動工（rAF 或 100ms 節流、加 attributes 監聽、評估超時降回 5-6 秒、白名單補欄位） |
| 38 | 未定 | — | 使用者實測 beta31 smoke（2026-08-14） | worker 視窗被最小化（macOS 收進 Dock）時，Chrome 凍結頁面繪製，點擊後的 dialog 流程停擺，每筆等滿超時後誠實失敗；放背景未最小化則正常。beta31 前的版本同樣受影響（且可能誤判）。應在 worker 文件 `visibilityState === 'hidden'` 時暫停佇列並提示，恢復可見再續跑，而非逐筆燒失敗 | 中 | 未動工。診斷證據：blocking-23c45694dc38（confirm 卡 21 秒、dialogCount=1、confirmDialogDetached=false、recheck=true，誠實失敗鏈完整運作） |
| 44 | 未定 | — | beta34 對抗性複核殘留（2026-08-15，**優先度下修**：#45 找到真根因後實機已正常，這三條未觀察到貢獻） | 貼文勾選框漏注入還有三條未觸碰的路徑：(a) `isInlinePostElementContext` 對非 article 結構可能判否；(b) MutationObserver 只對 addedNodes 觸發，純移除不會即時補掃，最快靠 1500ms backup interval；(c) dialog resolver 未識別時貼文路徑不補救。beta34 修的是 stale marker 與泛查詢誤跳過，這三條沒動 | 中 | 未動工。**beta34 已補齊各關卡診斷計數**，下一份實機診斷可直接定位是否有這三條的貢獻，不必再猜。證據見 `docs/history/2026-08-14-code-review-beta28-30.md` 同批複核與 beta34 複核報告 |
| 18 | 2.8.3-beta4 | v2.8.3 | 回報（使用者實測 2026-08-05） | 找不到連結／找不到帳號的失敗帳號，封鎖跑完後不會從佇列清掉，永遠留在開始封鎖清單 | 中 | 未動工（先取證） |
| 33 | 未定 | — | 使用者實測（2026-08-13，逐筆失敗清單累積 264 筆檢舉失敗） | 檢舉類失敗沒有批次處理工具：檢舉是否送達無法從頁面驗證，名單只能全部重試或全部清除，沒有逐批分流（例如按原因分組重試、按時間批次清理）。封鎖類已有「重新驗證」（beta28），檢舉類缺對等機制 | 中 | 未動工。前置事實：主要產生源頭（`missing_profile_root`、`missing_report_step`）已於 2.8.3 修掉，本項處理的是存量與工具面 |
| 27 | 未定 | — | 線上回報 #50／#51／#52（2026-08-05 至 08-06，2.8.2） | 三無掃描粉絲超過約兩千人就卡死：掃描視窗自行關閉、每掃一次跳一次個人頁、記憶體用盡到需強迫關機、停止並保留進度按了畫面不回來 | 大 | 等回報者定案，見下方第 27 節。**旁證**：疑似機制之一（勾選框寫入風暴）`2.8.3-beta6` 已修；另一疑似機制（結算撞 localStorage 配額無聲卡死）`2.8.4-beta22`（ADR 0023）已修。等回報者（hwid 681ade52）升級後回報 |
| 16 | 2.8.3-beta3 | v2.8.3 | 回報（使用者實測 2026-08-05） | 三無掃描結束後 worker 停在白畫面很久才跳出勾選畫面，期間無任何訊息 | 中 | **取證已落地**（`0a0d507`；該區間原本觀測點為 0，現有 10 個）；成因待實機 ring |
| 17 | 2.8.3-beta4 | v2.8.3 | 回報（使用者實測 2026-08-05） | 三無掃描吐 `followers_dialog_not_found` 後整趟結束；失敗原因不外顯、無診斷、無重試 | 中 | **取證已落地**（`c2b5343`）；重試段待實機失敗分布出爐後再定 |
| 23 | 2.8.3-beta3 | v2.8.3 | beta2 封鎖實測抓到（2026-08-05） | 選單開得起來但只有 7 項且無封鎖選項，等 9 秒後失敗，成因未知 | 中 | **取證已落地**（`0a0d507`）；成因待實機 ring |
| 3 | 2.8.2-beta3 | v2.8.2 | 遺留 | 每日上限提醒出現後仍繼續執行 | 小 | **已落地**（`28fef19`）；需跑到上限才看得到，待自然遇到時驗 |
| 8 | 2.8.3-beta10 | v2.8.3 | SSOT #1 | 「是否執行中」判斷 13 處分叉，stopping 被當成沒在跑 | 大 | **命名輪已落地**（`e411a91`；13 處改呼叫具名函式，行為零變化，80 格真值表對舊寫法全等）；**收斂待決**，見下方第 7 節 |
| 11 | 2.8.2-beta17 | v2.8.2 | SSOT #2 | 三個佇列集合在 core 散讀 53 處、各自組 Set | 大 | 未動工 |
| 12 | 2.8.2-beta18 | v2.8.2 | SSOT #3 | 平台上傳 payload 同一 UI 有兩份完整推導，欄位不一致 | 大 | 未動工 |
| 6 | 2.8.2-beta12 | v2.8.2 | SSOT #4 | dialog context 兩套競爭取法混用（first-vs-last 斷層線） | 中 | 未動工 |
| 7 | 2.8.2-beta13 | v2.8.2 | SSOT #5 | username 從 href 解析有多套互相矛盾的 parser | 中 | 未動工（順帶：`worker.js:1534` 的子字串路徑比對建議改用同檔 1653 的錨定 regex，code review 2026-08-14；另 SSOT 複盤 2026-08-15 發現字串正規化又長出三份本地版：`core.js:1450`／`1497` 逐字相同的內聯 fallback 與 `core.js:4467` 第三套，見快照 B4） |
| 9 | 2.8.2-beta15 | v2.8.2 | SSOT #6 | 貼文網址正規化有三套以上規則，同一貼文算成多則 | 中 | 未動工 |
| 10 | 2.8.2-beta16 | v2.8.2 | SSOT #7 | scroll root 三套選擇規則，錯 root 造成空清單或漏帳號 | 中 | 未動工 |
| 13 | 2.8.2-beta19 | v2.8.2 | SSOT #11 | skip-user 政策在過濾與 breakdown 各寫一份 | 中 | 未動工 |
...
| 1 | 2.8.2-beta1 | v2.8.2 | 回報 #47 | worker 視窗撐大與暫停用兩套尺量，特定環境永遠暫停；補 resize 取證＋修量法錯位 | 中 | **Fixed**（使用者實測通過，`7ac4af1`） |
| 2 | 2.8.2-beta2 | v2.8.2 | 遺留 | 冷卻保護搬走失敗清單後 UI 顯示 0，未說明資料去向 | 小 | **已作廢**（`b88ced6` 落地，但第 4 項停用冷卻後該情境不再發生，說明列已於 `3e7405b` 移除） |
| 4 | 2.8.2-beta4 | v2.8.2 | 查證 | 自動冷卻保護停用，只留提醒；舊備份給併回入口 | 中 | **Fixed**（`3e7405b`；2026-08-04 實機診斷確認封鎖流程正常、無冷卻觸發） |
| 4.5 | 2.8.2-beta5〜10 | v2.8.2 | 回報 | 查看動態各分頁勾選框重複繪製、位置錯誤、清單無限延伸（2.7.4-beta73 回歸） | 大 | **Fixed**（`625a6e1`／`ea60cbc`／`2b40ff0`；2026-08-04 使用者實機＋診斷驗收通過） |
| 5 | 2.8.3-beta3 | v2.8.3 | 遺留 | 檢舉偶發 `submit_not_confirmed`，實為第一次 action 後下一步沒出現 | 中 | **Fixed**（`0a0d507`；逾時改記 `missing_report_step`，該區間補上觀測點） |
| 14 | 2.8.4-beta30 | v2.8.4 | SSOT #12 | 48 小時保留窗硬寫 4 處 | 小 | **Fixed**（beta30；常數收斂到 config.js 單一定義，storage 兩個 ring 改引用，`grep "48 \* 60"` 歸零；`report-debug-context.js` 獨立 TTL 依計畫未合併） |
| 15 | 2.8.3-beta1 | v2.8.3 | 查證 | 正式版輕量診斷查不到根因：面板每 1.5 秒固定寫 2 筆洗掉 ring，且輕量層用 `slice(-120)` 位置切法丟掉 priority 保護過的關鍵筆 | 中 | **Fixed**（`5d9493d`，兩輪；2026-08-05 使用者實機 ring 驗收，失敗證據完整保留並據以查出第 19／20 項） |
| 19 | 2.8.3-beta2 | v2.8.3 | beta1 診斷實測抓到（2026-08-05） | 檢舉大量失敗實為 `missing_profile_root`，profile root 只查一次不等載入即放棄（4 至 9ms），且對外顯示成「找不到選單」誤導 | 中 | **Fixed**（`4d629a5`；2026-08-05 實機 ring 驗收，改後等滿 12,032ms 才放棄且標籤正確） |
| 20 | 2.8.3-beta4 | v2.8.3 | beta1 診斷實測抓到（2026-08-05） | `panel/clamp` 穩態噪音 | 小 | **Fixed**（`c2b5343`；實機 panel 從 109 筆降到 26 筆，clamp 只剩 2 筆） |
| 21 | 2.8.3-beta3 | v2.8.3 | beta2 封鎖實測抓到（2026-08-05） | `failure` 旗標只認三個字串，實際的失敗理由全部落在外面，分級仍是 0，長批次會被沖掉 | 小 | **Fixed**（`0a0d507`；改反向定義，語意來源收斂為一份） |
| 22 | 2.8.3-beta3 | v2.8.3 | beta2 封鎖實測確認（2026-08-05） | 封鎖對外仍回報「找不到選單」，即使診斷已正確記為 `missing_profile_root` | 中 | **Fixed**（`0a0d507`；重試資格與失敗清單行為不變，僅標籤改變） |
| 24 | 2.8.4-beta1 | v2.8.4 | beta2 實測抓到（2026-08-05） | 等滿 12 秒仍找不到 profile root，頁面真的沒 render | 中 | **Fixed**（`42651a4`；逾時後重載一次再試，並記錄救回率待實機驗證） |
| 25 | 2.8.4-beta30 | v2.8.4 | **beta4 實測抓到**（2026-08-05） | 噪音轉移到 `panel/reposition` 與 `selection/layout`：簽章含會變動的計數欄位，去重形同虛設 | 小 | **Fixed**（實為簽章 SSOT 重構時已修：漂移計數全在 `DIAGNOSTIC_SIGNATURE_MEASUREMENT_FIELDS`，簽章只投影狀態欄位。beta30 盤點全部簽章呼叫點確認無漏網，補回歸測試釘住；閒置 60 秒驗收待下次實機診斷順帶看） |
| 26 | 2.8.4-beta2 | v2.8.4 | 回報 #47 第二句 | 回報問題視窗訊息太長，送出按鈕被推到畫面外 | 小 | **Fixed**（`dc30726`；外框限高、內容區捲動、送出列 sticky） |
| 28 | 2.8.4-beta3 | v2.8.4 | Codex 複核 2.8.4-beta1（2026-08-08） | profile root 重試標記沒有時效檢查，worker 中途關閉後標記永久殘留，同一帳號下次直接被當成已重試而不給重載 | 小 | **Fixed**（2.8.4-beta3；加入 30 秒時效與無效標記清除） |
| 29 | 2.8.4-beta3 | v2.8.4 | Codex 複核 2.8.4-beta1（2026-08-08） | profile root 等待被速度模式倍率縮放，加速模式只等 4.8 秒。**2.8.3 起就存在，影響所有加速模式使用者** | 中 | **Fixed**（2.8.4-beta3；profile root 明確走不縮放等待） |
| 31 | 2.8.4-beta3 | v2.8.4 | Codex 複核 2.8.4-beta1（2026-08-08） | 重載發出到新頁接手的空窗期，停止指令無法取消已發出的 reload | 小 | **Fixed**（2.8.4-beta3；重載前與初始化優先檢查停止） |
| 32 | 2.8.4-beta5 | v2.8.4 | Codex 診斷複核（2026-08-10） | `renderTriggered` 在 reload 呼叫前只依 capability 寫成 true，若呼叫失敗或新頁未接手，診斷會誤稱已觸發 | 小 | **Fixed**（2.8.4-beta5；拆成要求／恢復，另記 unavailable／call failed） |
| 45 | 2.8.4-beta35 | v2.8.4 | 使用者實機回報（2026-08-15，繞五版才定位） | 個人頁往下滑，貼文的勾選框與命名標籤整排消失。根因是用「按鈕在畫面上緣以內」判斷標題區，捲動後貼文 top 變小甚至為負，該條件對負數永遠成立，於是每則捲過頂端的貼文都被判成標題區、蓋永久跳過印記；另一處對稱邏輯還會移除已注入的勾選框 | 中 | **Fixed**（beta35；兩處座標判定移除，改由結構式的 isInlinePostElementContext 判定，新增迴歸測試釘住。實測門檻 330，top=-370/-204/272 三則全漏、380/856 兩則正常，零例外。2026-08-15 使用者實機確認正常）。**教訓**：症狀是「捲動後消失」，第一輪就該懷疑座標判準；前四輪（beta32/33/34 兩輪）修的洞都真實存在但非本症狀成因 |
| 46 | 2.8.4-beta37 | v2.8.4 | 使用者實機回報（2026-08-15） | 「命名可疑」誤標真人：`sweetrice039220412`（顯示名「吳佳玲」，2.1 萬粉絲）與網軍帳號形狀相同，beta33 為了讓貼文卡片也能標記而採用帳號名形狀初判，形狀本身無法區分真人 | 小 | **Fixed**（beta37；形狀初判收回，改走「掃描已持久化旗標」與「個人頁標題取顯示名雙重比對」兩條有依據的路。順帶修正待審清單標籤強度分流：命名命中但非三無者不再標成「疑似假帳號」。2026-08-15 使用者實機確認正常） |
| 34 | 2.8.4-beta31 | v2.8.4 | code review 2026-08-14（beta28-30 範圍，對抗性驗證） | 封鎖假成功家族：(a) beta29 關閉偵測只盯確認框元素且判定先於限流檢查，確認框被限流視窗換裝時誤判成功、限流斷路器被推遲（`worker.js:2978`）；(b) beta28 複驗把「頁上零 dialog」單獨當成功證據，且 3 秒複驗後未再查限流（`worker.js:3041`） | 中 | **Fixed**（beta31；(a) check 先查限流再判移除，(b) 複驗後補查限流＋零 dialog 需確認框確實卸載，診斷加 confirmDialogDetached；換裝時序有測試釘住） |
| 39 | 2.8.4-beta32 | v2.8.4 | 使用者提供實機樣本（2026-08-14，四筆） | 網軍命名結構換代：顯示名去空白小寫恰為帳號名前綴，後接 2-4 字母亂碼與 5 碼數字（`Kenneth Berry` / `kennethberryaei31413`）。四筆全有盜用頭像，動物字典與 noAvatar 門檻都抓不到 | 中 | **Fixed**（beta32，ADR 0028；顯示名鏡像雙重吻合判定＋與拼音對稱進 isThreeNo，有活動者掛琥珀標籤。兩段式求值避免逐列 DOM 成本） |

版號欄是預期值；若某編號實際跨多個 beta（修壞重來），總表如實更新，編號不變。

第 34-37 項的完整發現內容（失敗情境、駁回項、修法建議）見 `docs/history/2026-08-14-code-review-beta28-30.md`；審查範圍是 beta28-30 的變更（`7777487^..1f72825`，src/ 六檔），非全 codebase。
...
### 2026-08-06 命名輪已落地（`e411a91`，`2.8.3-beta10`）

13 處硬寫的判定全部改呼叫 `src/utils.js` 的 `isBackgroundWorkerRunning`（10 秒）與 `isBackgroundWorkerBusy`（30 秒）。**行為零變化**，80 格真值表逐格與舊運算式對答案全等，另加 source-analysis 守門擋住未來再手寫一份。

**盤點結論修正**：原文寫 14 處，實際是 13 處。而且這 13 處在問**三個不同的問題**，不是同一個問題寫了 13 遍。

| 在問什麼 | 幾處 | 現行判準 |
|---|---:|---|
| 現在是什麼狀態（顯示用） | 3 | 10 秒 |
| 可以開新流程嗎（開 worker） | 7 | 10 秒 |
| 可以開新流程嗎（互斥閘門） | 3 | **30 秒** |

**那 30 秒不是寫錯，是刻意保守**，寧可多等也不要兩件事同時跑。原文「統一 freshness 門檻」的方向會提高撞車機率，**已作廢**。

**收斂未做，理由**：這 13 處分叉目前沒有任何實測症狀。唯一真實症狀是勾選框震盪，牽涉其中 2 處，而 `2.8.3-beta7` 動那 2 處就慢了十四倍並已退版（見第 24 節），成因至今未明。在查清楚前不再伸手。

**待使用者拍板的行為變化**（收斂時才會發生）：停止中目前 13 處全部當成沒在跑，改完之後停止中會擋下開新流程、匯入後不再詢問、面板顯示停止中；worker 卡在停止中時需要 stale 保險才能重開。
...
**修法方向待定**，可考慮的方向包含重載該分頁再試一次、拉長預算、或把這類帳號移到獨立的稍後重試清單。

### 2026-08-06 新增樣本：發生率會整段飆高

同一份匯出的舊區段（`2.8.3-beta7` 期間，約 6.5 分鐘）：11 個帳號中 7 個 `missing_profile_root`（各 12 秒），1 個 `menu_not_found`（32 秒），只有 3 個成功。發生率約六成，遠高於 beta4 的四十五分之一。

使用者當時回報「非常慢」。**慢的成因就是這一項**，每次失敗白花 13.5 秒且封不到人，不是流程本身變重。beta9 的分段計時證實正常情況每帳號 6 至 9 秒、未列帳時間僅 3 至 11 毫秒。

**成功與失敗是二元的**：成功案例 root 在 3 至 20ms 內出現，失敗案例整整 12 秒零候選。沒有中間值。

**觸發條件仍未查明。** 兩個假設待驗：平台端當下的自動化阻擋、或該版本自身的影響。使用者停止後重開，同一批帳號即恢復正常。同一天稍晚的 `2.8.3-beta9` 實機 11 個帳號，此失敗 0 次。**下次再遇到明顯變慢時直接看 `navigationToRootMs`，12 秒即為本項，可與其他成因區分。**
...
## 28. profile root 重試標記沒有時效（Codex 複核 2.8.4-beta1 抓到）

`Worker.getProfileRootRetryAttempt`（`src/worker.js:152-162`）讀取 `hege_profile_root_retry` 時只比對 `user` 與 `mode`，**完全不檢查 `requestedAt`**，雖然 `markProfileRootRetry`（`src/worker.js:164-172`）有寫入該欄位。

正常路徑會在第二次等待開始前清除標記，所以單一 worker 跑完不會殘留。**但 worker 在標記寫入後、清除前中途關閉時，標記會永久留在 localStorage。**

後果：該帳號下次被處理時直接回傳 attempt 2，只等一輪就寫入 `missing_profile_root`，**失去第 24 項承諾的重載重試**。而會走到這條路徑的帳號，正是最需要重試的那些。

**修法方向**：讀取時檢查 `requestedAt` 時效，超過合理視窗即視為無效並清除。時效長度需涵蓋一次頁面重載加第二輪等待，且要考慮第 29 項的速度模式縮放。
...
`PROFILE_ROOT_WAIT_MS` 是 12000，實際等待因此變成：
...
**這不是 2.8.4 引入的，第 19 項在 2.8.3-beta2 落地時就已經如此。** 我方在第 19 與第 24 項的敘述一律寫「等滿 12 秒」，對加速模式使用者從未成立。

`Utils.safeSleep`（`src/utils.js:170-171`）的註解已載明「頁面載入 fallback 不受速度模式影響」，**profile root 等待是頁面載入等待，沒有照這條原則做**。

實機 ring 觀測到的 `waitMs` 落在 12013 至 12101，代表該回報者使用智慧或標準模式。**加速模式使用者的失敗率應該更高，目前沒有樣本。**

**修法方向**：讓 profile root 等待不受速度模式縮放，比照 `safeSleep` 的既有原則。

**注意**：這會讓加速模式使用者的等待從 4.8 秒變成 12 秒，單一失敗帳號的成本上升，換取誤判失敗減少。**這是行為變化，需要使用者拍板。**
...
**依附第 8 項。** Codex 同時指出 `_stepRunning`（`src/worker.js:1475`）只是單一分頁內的 mutex，兩個 worker 本來就可能取到同一個佇列頭並各自增加 `stats.failed`，`Worker.saveStats`（`src/worker.js:85`）又是非原子覆寫。**多分頁同時跑本身就已經壞了，本項只是在已壞情境上多加一次重載。**

**修法方向**：標記加上分頁 owner 或 batch 識別。真正的解法在第 8 項的跨分頁 worker lock。

## 31. 重載空窗期停止指令無法取消（Codex 複核抓到）

`location.reload()` 是非同步的，發出後呼叫端立即結束本次 `runStep`，不再排下一次輪詢。這段空窗中停止按鈕只寫入 `BG_CMD=stop`，無法取消已發出的 reload，必須等新頁載入初始化後才處理停止。

第二輪 12 秒等待本身**可以**中止，約每 100ms 檢查一次。問題只在重載轉換的那一兩秒。
...
## 27. 三無掃描粉絲超過約兩千人就卡死（線上回報 #50／#51／#52）

**同一位回報者在兩天內連續回報三次**，`hwid` 681ade52，全部是 `2.8.2`、Win32、Chrome 109。目前清單上最痛的一項，該功能對他等於不可用。

### 使用者描述（#51 最完整）

1. 粉絲超過兩千人，自動掃描直接卡住無法繼續
2. 掃到某個帳號時掃描分頁自己關掉，畫面停在該帳號的個人頁
3. **超過兩千人之後，每掃一次就跳一次個人頁，這是本來不會發生的**
4. 當機到電腦不能用必須強迫關機，中途出現記憶體用盡提示
5. 「停止並保留進度」按了三次都卡住，要關掉分頁 Chrome 才正常
6. 不停留在掃描分頁、去別的分頁操作時，掃描也會 lag 卡住

#52 補充：掃到 2300 人時跳到追蹤者個人頁，順利就繼續、不順利就卡死，關掉重來後完全無法再掃。

### 診斷附件的實際內容（已讀完三筆）

| 能確定的 | 對應描述 |
|---|---|
| 掃描視窗開了又消失，連續開三次都沒有正常結束的紀錄 | 第 2 點 |
| 兩次視窗在 12 到 15 秒就逾時關掉 | 「關掉重來後無法執行」 |
| 一次跑了 15 分鐘才被停止，另兩次各約 11 分鐘 | 第 1 點 |
| **停止確實有被記錄到** | 第 5 點的按鈕**有反應**，問題在按下去之後畫面沒有恢復 |

**取證缺口，這是本項目前最大的問題：**

- 捲動次數、已檢查人數、佇列人數**全部都是 0**，看不到掃到第幾人才死，兩千這個門檻完全對不上證據
- 沒有任何記憶體或規模的數字
- **#51 那三百筆紀錄裡三無一筆都沒有**，全被 `report`／`panel`／`message_route` 的雜訊洗掉。他寫得最詳細的一筆，診斷最沒用

### 一個尚未證實的機制假設

2.8.2 的 `syncCheckboxQueueState` 每一列勾選框都會重建封鎖名單，每列兩次同步 storage 寫入。粉絲名單對話框是一路往下長的，**兩千列等於每 1.5 秒約四千次同步寫入**，會卡住整個分頁。這個代價跟列數成正比，少的時候感覺不到，正好對得上「超過兩千才開始壞」。

`2.8.3-beta6`（`4708ec2`）已修掉這一項，實測從每三輪 288 次寫入降到 0。

**但這只是機制對得上，不是證實。** 沒有該回報者在 2.8.3 之後的任何紀錄。而且視窗自己關掉與停止後畫面不回來這兩點，2.8.3 完全沒碰。

### 下一步

**最省事的驗法是請該回報者裝 2.8.3 以上再跑一次。** 若卡頓消失即確認；若仍卡，也會拿到品質好很多的診斷，因為雜訊洗掉三無紀錄的毛病 2.8.3 已修。這比先補插樁再出一版快。

**若要自行修**：先補掃描進度插樁，捲到第幾次、檢查到第幾人、視窗還在不在，這三個現在全是 0，不補就永遠只能猜兩千這個數字。做法比照第 24 項，先量再修。
