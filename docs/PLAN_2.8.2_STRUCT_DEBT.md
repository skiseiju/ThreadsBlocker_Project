...
| 編號 | 版本 | 來源 | 一句話 | 規模 | 狀態 |
|---:|---|---|---|---|---|
| 1 | 2.8.2-beta1 | 回報 #47 | worker 視窗撐大與暫停用兩套尺量，特定環境永遠暫停；補 resize 取證＋修量法錯位 | 中 | **Fixed**（2026-08-03 使用者實測通過，commit `7ac4af1`；外框上下界移除、viewport 下界吸附＋debounce） |
| 2 | 2.8.2-beta2 | 遺留 | 冷卻保護搬走失敗清單後 UI 顯示 0，未說明資料去向 | 小 | 已落地待驗（commit `b88ced6`；需真的觸發 Meta 冷卻才看得到，使用者自然遇到時再驗） |
| 3 | 2.8.2-beta3 | 遺留 | 每日上限 250/200 提醒出現後仍繼續執行 | 小 | **已落地**（commit `28fef19`；使用者拍板維持繼續執行，只把提醒改為 worker compact 與主面板都看得到）待實測 |
| 3.5 | 2.8.2-beta4 | 回報／查證 | 自動冷卻保護停用，只留提醒；舊備份提供併回入口 | 中 | **已落地**（commit `3e7405b`）待實測 |
| 4 | 2.8.2-beta4 | 遺留 | 檢舉偶發 `submit_not_confirmed`，取證不足 | 中（先取證） | 未動工 |
| 5 | 2.8.2-beta5 | SSOT #4 | dialog context 兩套競爭取法混用（first-vs-last 斷層線） | 中 | 未動工 |
| 6 | 2.8.2-beta6 | SSOT #5 | username 從 href 解析有多套互相矛盾的 parser | 中 | 未動工 |
| 7 | 2.8.2-beta7 | SSOT #1 | 「是否執行中」判斷 14 處分叉，stopping 被當成沒在跑 | 大 | 未動工 |
| 8 | 2.8.2-beta8 | SSOT #6 | 貼文網址正規化有三套以上規則，同一貼文算成多則 | 中 | 未動工 |
| 9 | 2.8.2-beta9 | SSOT #7 | scroll root 三套選擇規則，錯 root 造成空清單或漏帳號 | 中 | 未動工 |
| 10 | 2.8.2-beta10 | SSOT #2 | 三個佇列集合在 core 散讀 53 處、各自組 Set | 大 | 未動工 |
| 11 | 2.8.2-beta11 | SSOT #3 | 平台上傳 payload 同一 UI 有兩份完整推導，欄位不一致 | 大 | 未動工 |
| 12 | 2.8.2-beta12 | SSOT #11 | skip-user 政策在過濾與 breakdown 各寫一份 | 中 | 未動工 |
| 13 | 2.8.2-beta13 | SSOT #12 | 48 小時保留窗硬寫 4 處 | 小 | 未動工 |
