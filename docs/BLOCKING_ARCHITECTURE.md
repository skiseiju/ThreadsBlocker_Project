...
4. 粉絲列表會先做候選預篩：列表無可見頭像者優先進 profile 檢查；Threads / Instagram 匿名預設頭像（例如 `anonymous_profile_pic` / `ig_cache_key=YW5vbnltb3VzX3Byb2ZpbGVfcGlj` / static CDN fallback `5OTfmveiK1K.jpg`）必須視為無頭像；列表已有非預設可見頭像但 username 符合預設「動物字詞 + 數字亂碼」者，也會作為次要候選進 profile 檢查；列表已有非預設可見頭像且 username 正常者會寫入本批 cursor 但不進 profile。文章旁勾選框旁的本機假帳號 badge 也重用這組命名規則與既有三無待審清單，只做命名提示，不會自動封鎖、抓地區時間或啟動三無掃描；可見 username / profile link 旁不再另畫 badge，避免同一帳號重複提示。
...
收集中按下「停止並結算」時，第一輪會保留當輪已驗證帳號；第二輪會把第一輪與第二輪目前已驗證帳號去重合併，再加入上述兩份清單。只有使用者手動停止可做這種部分結算；逾時、DOM schema 不明、排序失敗、截斷或超限仍整批 rollback，停止時 0 人也不做空提交。
