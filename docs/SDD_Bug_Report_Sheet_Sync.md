...
GitHub PLAN markdown  ← 工作項唯一來源（SSOT）
      ...
      ▼
「工作項」分頁（整頁清空後重寫）＋「回報」K 欄（PLAN 對應工作項）
```

資料流為單向：D1 →「回報」、GitHub PLAN →「工作項」與「回報」K 欄。試算表是唯讀鏡像；「回報」I／J 仍是人工處理欄，任何內容都不回寫 D1 或 GitHub。
...
| J | 備註 | **人工** | 自由填寫 |
| K | 對應工作項 | 腳本 | PLAN「來源」欄提到的回報 ID 對應之工作項編號；以 `, ` 分隔，無對應留空 |

**腳本新增列時寫 A–H；既有「回報」列只允許更新 H。** A–G、I、J 永不回頭修改；I、J 由人工維護，腳本不得讀取或覆蓋。K 是獨立的腳本欄，`syncWorkItems()` 會整欄重寫；只讀 A 欄作回報 ID 對照，不寫回 A–H，找不到對應回報列時略過。舊分頁若尚無 K 標題，腳本只補上 K1「對應工作項」。
...
解析總表後，腳本逐列掃描「來源」欄；只有來源文字含「回報」時，才以 `/#(\d+)/g` 擷取該欄所有回報編號，建立「回報 ID → 工作項編號清單」對照。因此 `回報 #47`、`回報 #47 第二句`、`線上回報 #50／#51／#52` 會擷取，`SSOT #4`、`beta1 診斷實測抓到` 不會擷取。同一工作項重複提及同一回報只保留一次，清單順序沿用 PLAN 總表。接著只讀「回報」A 欄，將對應編號以 `, ` 串接寫入 K；K 整欄包含無對應列都會重寫為空白。PLAN 提到但「回報」A 欄不存在的 ID 直接略過，不報錯；I、J 不讀寫，A–H 只讀 A、不寫回（H 仍由 `refreshBackendStatuses()` 管理）。
...
| PLAN「來源」提到不存在的回報 ID | 不在「回報」新增列；K 對應略過且不報錯。 |
