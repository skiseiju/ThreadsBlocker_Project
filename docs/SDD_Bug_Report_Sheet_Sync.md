...
| 欄 | 標題 | 寫入者 | 內容 |
|---|---|---|---|
| A | 回報 ID | 腳本 | `report_id`，去重鍵 |
| B | 收到時間 | 腳本 | 台北時間 `yyyy-MM-dd HH:mm` |
| C | 類型 | 腳本 | `type` + `error_code` |
| D | 版本／平台 | 腳本 | `version` / `platform` |
| E | 問題描述 | 腳本 | `message` 全文 |
| F | 診斷附件 | 腳本 | 有／無 |
| G | 處理狀態 | **人工** | 待處理／處理中／已回覆／不處理 |
| H | 備註 | **人工** | 自由填寫 |

**腳本只寫 A–F，且只在 append 新列時寫入。腳本永遠不修改既有列的任何欄位，包含 A–F。** G、H 由人工維護，腳本不得讀取或覆蓋。

新列的 G 欄預設填「待處理」，僅在該列首次建立時寫入一次。
