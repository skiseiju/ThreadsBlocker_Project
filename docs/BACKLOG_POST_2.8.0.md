...
### F10 — 勾選封鎖對象時一併封鎖按讚者（線上回報 #55，2026-08-15）

使用者要的是「在串文原地把按讚的人一起收進封鎖名單」。現況要進「查看動態」才做得到，
但那會導致整串文的**閱讀位置重置**，看到一半的位置跑掉，這才是他真正的痛點。

兩種方向，動工前先決定：

1. **原地收集**：在串文列表就近抓按讚名單，不離開當前頁面。技術上要處理按讚名單是
   lazy render 的對話框，等同把清理名單的收集流程搬到串文情境，規模中等。
2. **保留閱讀位置**：維持現有的「查看動態」路徑，但進去前記住捲動位置、回來時還原。
   規模小很多，且能一併解掉其他需要離開列表的流程的同款困擾。

**建議先做第 2 案**：成本低、受益面廣，且若做完使用者仍覺得不便，再評估第 1 案。
