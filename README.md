...
*   **Chrome 三無追蹤者掃描**：
    ...
    *   掃描以 Threads worker 分頁執行，支援本機 cursor 接續、可調整備選門檻，完成後累加到管理清單；可用三無原因、新帳號時間、國家/地區（含未分享）、掃描來源與掃描日期做多重 filter，再批次清除、加入安全名單或加入封鎖清單。
    *   帳號不公開會以獨立標籤呈現，不會同時標成無發文、無回文或無轉貼；加入封鎖清單後仍需由使用者手動啟動封鎖 worker。
    *   三無名單只保留在本機；平台同步只上傳檢查數、符合數與掃描狀態等匿名 aggregate 統計。
*   **只檢舉與資料上傳**：
