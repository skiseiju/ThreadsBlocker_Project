## v2.8.4-beta15 — 三無停止會保留目前待審清單

* **TL;DR：三無掃描按下停止後會立即結算目前 findings、保留可續掃游標，並自動打開已掃到的待審清單，不再等同取消。**
* **停止重載修正**：worker 若在停止指令送出後剛好重載，會在 owner 驗證後以 `stopped` 做一次冪等結算；不再因 `stopping` 提前返回而跳過結果持久化。
* **報告可見性**：有資料的 `stopped` 報告與正常完成報告一樣可自動開啟，beta 升版也會保留；空的停止結果不會顯示空白報告。
* **行為不變**：只保存已確認為三無的待審結果，不自動加入封鎖清單；尚未處理的候選人保留在續掃游標。
* **回歸契約**：鎖定停止狀態重載必須進入一次 `finishScan(status=stopped)`，且只有含 findings 的停止結果可進入報告。

## v2.8.4-beta14 — 大型清理名單掃描加速

* **TL;DR：收集按讚名單時暫停背景 checkbox 掃描，結束後只補掃一次；200 人後不再因每次捲動反覆重掃整份 dialog 而越來越慢。**
* **根因修正**：`injectDialogCheckboxes()` 改為每輪只建立一次 username → checkbox 索引，移除每個帳號列都查詢整個 dialog 的 O(n²) 路徑。
* **收集期間靜默**：scroll listener、MutationObserver 與 1500ms backup scanner 在單輪 collector 活躍時直接略過；成功、失敗、手動停止或例外離開後都會解除狀態，最外層結束只排一次補掃。
* **行為不變**：兩種排序各掃一輪、最後進度後 5 秒停止、1000 人上限、atomic rollback 與「停止並結算」保留已收集名單均維持原規則。
* **回歸契約**：200 列 fixture 鎖定每輪整個 dialog checkbox 查詢由 200 次降為 1 次，並驗證 nested 收集期間 scanner 為 0 次、最外層離開只補掃 1 次。

## v2.8.4-beta13 — 每日封鎖上限提醒縮短

* **TL;DR：移除每日上限警告的重複說明，只保留近 24 小時計數、開始釋放時間、可選的舊版估計截止與「超限仍會繼續」。**
* **顯示範例**：`⚠️ 近24h 234/200。8/11 19:30 起逐筆釋放；舊版估計 233 筆將於 8/11 23:08 前清完。超限仍會繼續。`
* **行為不變**：success-only 計數、rolling 24h 逐筆退出與只提醒不強制停止的規則均維持 beta12 行為。
* **回歸契約**：固定時間 fixture 鎖定有／無舊版估計兩種短文案，完整訊息不得超過 80 字元，舊說明段落不得回流。

## v2.8.4-beta12 — 每日封鎖計數改為成功結果與 rolling 24h 釋放提示

* **TL;DR：每日安全估計不再把每次 worker 嘗試都寫成「已封鎖」；只有本次 `autoBlock()` 回報成功、且不是解除封鎖才計數，有抽樣驗證的帳號則等驗證通過才寫入。**
* **排除假計數**：`already_blocked`、`already_unblocked`、失敗、限流、找不到頁面／選單、私人帳號與解除封鎖都不再增加每日封鎖數；一般成功、inline 驗證成功與 batch 驗證成功是僅有三個寫入點。
* **24 小時不是午夜歸零**：每筆成功紀錄在自己的時間滿 24 小時後逐筆退出。超限提醒會顯示最早一筆的退出時間；主面板同步顯示相同精簡提示。
* **舊資料保守遷移**：既有數字 ring 無法分辨成功與失敗，不冒險整批刪除；beta12 首次讀取會標出舊版估計筆數與最晚退出時間，最多再經一個 rolling 24h 後完全轉為 success-only。超過 48 小時及未來時間戳會清除。
* **回歸契約**：功能測試固定時鐘驗證逐筆到期、未來時間排除、舊／新計數邊界；source contract 鎖定三個成功寫入點並禁止結果分流前無條件計數。

## v2.8.4-beta11 — 停止掃描會保留已抓到的名單

* **TL;DR：按下「停止並結算」不再整批取消；第一輪停止會直接保留當輪已抓到的人，第二輪停止則把第一輪與第二輪目前結果去重合併後加入封鎖／檢舉清單。**
* **停止不是假完成**：結果仍標示 `complete=false` 與 `reason=stopped`，只有清理名單 orchestrator 明確產生、至少包含一個帳號且未超過安全上限的 `partialCommit` 才能通過提交閘門。
* **錯誤仍會回滾**：`scroll_stall`、timeout、unknown、排序切換失敗、截斷與超限維持整批 rollback；停止前尚未抓到任何帳號也不做空提交。
* **回歸契約**：fixture 實際點擊第一輪「停止並結算」，確認不切排序且名單可提交；另驗證第二輪停止會合併兩輪、大小寫不敏感去重，以及未明確 opt-in 的 `stopped` 仍不能繞過 atomic gate。

## v2.8.4-beta10 — Likes 兩種排序各掃一輪

* **TL;DR：清理名單先保持目前排序掃到連續 5 秒沒有新帳號，再切換排序掃第二輪，最後把兩輪名單去重合併。**
* **真的換排序**：第一輪若是「預設」，第二輪切「最新」；第一輪若已是「最新」，只在 scoped 選單恰有兩項時切到唯一另一項。每次切換仍重新開選單驗證，第一次 click 未生效會自動重抓 trigger 再試一次。
* **兩輪 atomic**：第一輪、排序切換與第二輪都必須完整成功才提交；第二輪停滯、出現 unknown、超過安全上限或切換失敗時，第一輪結果也不會送出，沿用既有 rollback。
* **回歸契約**：fixture 證明第一輪先跑過 idle deadline 才切排序、83 + 140 去重為 141；第二輪不完整會回傳空名單，Default ↔ Latest 雙向切換與 beta9 bounded retry 也保留。
...
* **實機根因**：同一篇 1,742 個讚的貼文在「預設」排序捲到底並等待 6.5 秒後仍停住；切成「最新」後可繼續超過 150 列，證明不是等待秒數不足。
* **限縮範圍**：只有「清理名單」要求最新排序；定點絕／reservoir 等共享 collector caller 不變。排序按鈕限縮在目前 Likes dialog，portal menu 只接受本次新出現的精確「最新」選項。
* **完整性保護**：舊版面沒有排序控制時維持相容；已看到排序控制卻無法選取或驗證「最新」時，整次收集以 `likes_sort_switch_failed` 回滾，不會把 82 人誤當完整名單送出。
* **回歸契約**：beta99 red-first fixture 修正前 0/2、修正後 2/2；fixture 覆蓋 scoped dialog、portaled menu、選取驗證與 82→140 收集。
...
* **自動續抓**：`collectFullDialogUsers()` 改以「最後一次進度」計時；新帳號、`scrollTop` 推進或 `scrollHeight` 增長都會重設等待時間。
* **停止與邊界**：連續 5 秒無進度且位於底部才回報 `end`，未到底則保留 `scroll_stall`；既有 1000 人安全上限、800 次捲動上限與手動停止行為不變。
* **回歸驗證**：新增延遲 1.2 秒才出現下一批 Likes 的 production-path fixture，並驗證真正無資料時會等滿設定的無進度期限。

## v2.8.4 — 個人頁未 render 時重載重試

* **TL;DR：頁面沒有 render 出個人頁時，封鎖與只檢舉各重載當前分頁一次再試；診斷會記下第一次逾時、重試次數與第二次結果。**
* **個人頁載入重試**：profile root 等滿 12 秒仍不存在時，沿用 `history.replaceState` 加 `location.reload()` 重載當前分頁一次；第二次仍沒有 root 才列入 `missing_profile_root`。404、限制訊號與其他失敗理由維持原本的提前返回與佇列行為。
* **停止與診斷**：重載前後沿用 `BG_CMD=stop` 停止機制；停止會在輪詢下一個檢查點立即中止。每個帳號最多新增兩筆 root 診斷，包含第一次逾時與第二次成功或失敗。
* **問題回報視窗版面**：回報視窗改用限高與欄式版面，標題列和送出列固定，中間內容可捲動；矮視窗也能看到送出回報按鈕。
* **測試版號契約**：clean-list 診斷測試改用通用版本格式守門，正式版與後續 beta 版不會因跳版號失效；beta 診斷閘門維持原本規則。
* **profile root 等待**：封鎖與只檢舉共用的頁面載入等待固定為 12 秒，四種速度模式都不套用倍率；加速模式每輪等待由 4.8 秒改為 12 秒，降低頁面尚未 render 就被判定失敗的機率。
* **重試標記時效**：profile root 重試標記加入 30 秒時效，缺漏、格式錯誤、未來時間與逾時標記都會清除並重新取得兩輪機會。
* **停止與重載**：profile root 重載發出前與新頁初始化時都優先檢查停止指令，收到停止後沿用既有停止收尾，不會發出或進入第二輪等待。
