## v2.8.2 — beta7 預覽卡多輪注入維持單一勾選框

* **TL;DR：beta7 修正緊湊預覽卡在重複掃描時每輪新增勾選框的回歸，引用與讚分頁連續注入五輪後都維持單一框。**
* **掛載點去重**：去重搜尋同時涵蓋解析列、實際掛載 host 與同列 rowId；預覽框掛在完整列時，後續掃描會重用原框，不會並排累積。
* **既有行為保留**：一般貼文卡、有追蹤按鈕帳號列與預覽卡都維持一列一框；低信心框仍可在後續高信心解析時搬移，預覽卡維持 24x24 與列尾端對齊。
* **分頁與幾何驗證**：引用、讚兩種 dialog 標題均加入五輪斷言；讚分頁預覽卡的 ID 文字與勾選框矩形不重疊。

* **TL;DR：beta6 將無追蹤按鈕貼文卡的勾選框統一推到列尾端，並讓緊湊預覽卡使用 24px 變體，避免壓到 ID、箭頭、被引用者、時間與編輯圖示。**
* **貼文卡尾端掛載**：貼文卡勾選框改為列尾端掛載，保留追蹤按鈕前的插入點；沒有追蹤按鈕時以 `margin-left:auto` 對齊列尾端，ID 長度不再改變框的水平位置。
* **緊湊預覽卡**：沿用 beta5 的 `bounded_parent` 低信心列判定，將掛載 host 提升到完整預覽列，避免框留在 ID 包裝層；預覽情境新增 24x24 尺寸變體，預設 36x36 尺寸不變。
* **排版保護**：注入時只補既有 host 所需的 flex、對齊與間距屬性，不使用絕對定位；重複 pass 會把尾端框搬回正確位置。
* **驗證**：新增貼文卡與緊湊預覽卡幾何 fixture；修正前貼文卡右緣 spread 為 92px，預覽框與箭頭及被引用者重疊，修正後右緣一致且相鄰矩形零重疊。

* **TL;DR：beta5 修正查看動態的讚、引用與轉發分頁重複繪製、清單延伸與位置偏移；每張貼文卡只保留行為者勾選框。**
* **單一列解析器**：`injectDialogCheckboxes` 改由 `Core.resolveAccountRowContext` 判定帳號列、貼文卡、掛載節點與信心等級；引用與轉發不再各自維護分頁特例。
* **安全列界線**：追蹤按鈕向上尋找列容器時加入高度守衛，拒絕數千像素的共用祖先；拿不到可信列時保留巢狀 dialog 的有界低信心救援，等待後續 pass 重試。
* **可自我修正**：低信心既有框會在取得較高信心列容器後搬移，重複 pass 不會增加框或持續改變捲動高度；診斷只保留取樣列規則、信心與搬移數字／布林。
* **驗證**：新增引用、轉發、讚分頁時序與巨大祖先 fixture；修正前每張貼文卡觀測到 3 個框，修正後每張卡 1 個，行為者位置在名稱右側。

* **TL;DR：beta4 停用冷卻保護自動觸發，命中疑似平台限制時只提醒並記錄失敗；既有冷卻備份可從主面板併回待處理清單。**
* **beta4 冷卻保護停用**：連續驗證失敗、選單異常或平台限制訊號不再自動進入 12 小時冷卻、不搬移名單、不回滾資料庫；畫面與 worker 紀錄會提醒手動暫停。
* **beta4 舊備份併回**：保留既有 `COOLDOWN_QUEUE`，主面板只在有資料時提供「把冷卻備份併回待處理清單」入口，併回後清空該 key。
* **beta1 視窗尺量**：修正 worker 視窗以內容區缺口補足尺寸，並補上 resize 診斷取證。
* **beta2 冷卻備份提示**：舊版曾在冷卻保護搬移失敗清單後顯示已備份筆數；beta4 改為提供舊資料併回入口。
* **beta3 每日上限提醒**：worker 縮小模式與主面板都會醒目顯示已封鎖數、自訂安全估計值及可能的平台限制；提醒只提供風險資訊，流程仍會繼續執行。
