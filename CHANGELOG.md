...
* **TL;DR：清理名單會把按讚名單的兩種排序各掃一輪並去重合併；停止時保留已抓到的帳號，三無掃描也會保留目前結果並自動開啟待審清單。**
* **清理名單**：按讚名單依目前排序與「最新」排序各掃一輪，再以帳號去重合併，減少漏人；按下「停止並結算」時保留已抓到的帳號，不再整批取消；大型名單收集期間暫停背景勾選框掃描，完成後補掃一次，操作不再逐輪變慢。
* **每日上限**：只有真正封鎖成功的結果會計入近 24 小時上限，失敗、已封鎖、已解除封鎖與其他未成功結果不會增加數字；每筆紀錄滿 24 小時逐筆退出，提醒會顯示下一筆釋放時間。
* **三無掃描**：按下停止後立即結算並保留續掃位置，自動開啟已掃到的待審清單；掃描期間暫停背景勾選框掃描。
* **封鎖與只檢舉**：個人頁尚未 render 時會重載一次再試；頁面載入等待不受速度模式縮放；回報視窗的送出按鈕固定可見。
* **診斷改善**：輕量診斷持續收集，並將重載、流程結束與失敗原因整理得更容易追查。
