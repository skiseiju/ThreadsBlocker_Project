## v2.8.3 — 診斷噪音收斂、重要紀錄挑選、Worker 提前落盤

* **2.8.3-beta10 背景 worker 判定語意命名**：將 10 秒的一般判準與 30 秒的保守判準抽成具名常數與函式，13 個呼叫點只改用具名函式，數值、條件與流程維持不變。
* **2.8.3-beta9 逐帳號分段計時與執行邊界取證**：封鎖帳號診斷新增導頁到 root、root 到選單、選單到動作、動作到確認與總耗時，並計算未列帳時間；匯出摘要標示最近一次封鎖或檢舉執行起點前後的條目數，舊 ring 紀錄完整保留。
* **2.8.3-beta6 勾選框掃描不再重建封鎖 session**：session latch 只在真正啟動封鎖時建立，逐列勾選框只使用同一輪 scanner pass 的狀態快照；勾選顯示、停止後殘留選取與 latch 行為維持原規則。
* **2.8.3-beta6 收窄正常完成診斷豁免**：priority 4 失敗紀錄仍完全保留，priority 2 啟動與 priority 3 正常完成改用每分鐘 60 筆的獨立額度，普通紀錄維持每分鐘 22 筆。

* **2.8.3-beta5 診斷簽章全面排除量測值**：panel、selection、route、clean-list、followers 與三無診斷共用狀態簽章 helper；`menuItems`、`candidateCount`、`accountRowCount`、`dialogCount`、時間與各種計數仍保留在 fields，但不再造成穩態重複紀錄，對話框數量降維為有無狀態。
* **2.8.3-beta4 面板與三無粉絲入口取證**：面板夾位診斷只在整數化位置、尺寸、視窗或夾位狀態變化時記錄；三無粉絲入口的四種取法、輪詢耗時、兩次點擊與對話框結果補進有界診斷，失敗畫面會說明卡住步驟。
* **2.8.3-beta3 第二輪三無收尾診斷**：補上掃描完成、結果彙整、儲存落盤、報告渲染觸發與統計上傳等待的數字／布林觀測，條目數量固定受控且不記錄帳號或頁面內容。
* **2.8.3-beta3 失敗分級與 root 標籤**：明確成功與既有狀態略過共用同一份失敗語意，其他結果一律保留失敗優先級；封鎖 profile root 逾時改回傳「帳號頁面尚未載入完成」，既有選單失敗資料仍可顯示與重試。
* **2.8.3-beta3 檢舉第二步診斷**：第一次檢舉動作後補上等待觀測，第二步逾時改記「檢舉下一步尚未出現」，並保留選單類別、root 結構與閘門狀態的數量／布林取證。
* **2.8.3-beta2 檢舉 profile root 載入等待**：檢舉在 profile 模式會等待目標帳號頁面完成載入，逾時改記為「帳號頁面尚未載入完成」，不再誤顯示找不到選單；封鎖側 root 診斷也沿用相同語意。
* **TL;DR：收斂閒置面板與路由診斷噪音，輕量診斷優先保留封鎖生命週期紀錄，Worker 啟動或關閉前都會保存診斷，讓正式版問題回報能取得啟動與結束線索。**
* **穩態診斷收斂**：面板隱藏狀態、路由類型、整數化位置與視窗尺寸沒有變化時不再重複寫入；MutationObserver 只觸發掃描，真正的狀態變化仍會記錄。
* **重要紀錄優先**：輕量層先保留 priority 紀錄，再以最近的普通紀錄補足 120 筆，輸出順序仍依時間排列。
* **失敗證據優先**：failure、breaker、error、timeout 與 rollback 分到最高層，輕量層溢位時優先保留，並優先補回相同 operationId 的上下文。
* **通用診斷節流**：同 feature 與 stage 的一般紀錄每分鐘最多寫入 22 筆，超過後合併 repeatCount；失敗與 terminal 紀錄不受此上限影響。
* **Worker 診斷落盤**：Worker 啟動、佇列取件、封鎖閘門、terminal/error 與 pagehide/beforeunload 都會保存診斷；一般佇列取件採 500 毫秒最短間隔，啟動與結束事件不受節流。
