> **目前公開版本：v2.8.3**（2026-08-06，CWS／AMO／GitHub）。
> 以下 `v2.8.4` 與其所有 `v2.8.4-betaN` 條目**尚未對外發布**，仍在開發中。
...
## v2.8.4 — 清理名單行為修正、停止按鈕行為修正、每日上限計數修正

> **尚未發布。** 本節寫於 2026-08-11，內容只涵蓋 beta1 至 beta16；
> 其後累積的 beta17 至 beta37（三無判定重寫、封鎖誤判修正、勾選框注入
> 修正等）也會併入同一個正式版，發版前需重新整併本節。

* **TL;DR：這版主要在修「抓不齊」跟「按了停止就全沒了」這兩件事，順便把每日上限算錯的數字修好。**
* **清理名單以前會漏人**：按讚名單只掃一種排序，沒掃到的就當作不存在。現在兩種排序都掃過再合併，重複的自動去掉。
* **按停止不再等於整批取消**：以前按「停止並結算」，已經抓到的人會一起消失，等於白做。現在按下去就把抓到的留給你。
* **名單很大時不會越跑越慢**：以前每滑一次就把整份名單重新檢查一遍，兩百人以上會明顯卡。現在收集期間先停掉背景掃描，結束後補一次就好。
* **每日上限的數字以前算錯**：不管成功或失敗都算一筆，連「本來就封鎖過」跟「解除封鎖」都算。所以提醒常常太早跳出來。現在只有真的封鎖成功才算，而且每筆滿 24 小時就自己退掉，不是等午夜歸零。
* **三無掃描按停止會保留進度**：立刻把掃到的結果存起來，記住掃到哪裡，並直接打開待審清單給你看。
* **封鎖遇到頁面沒載出來會再試一次**：以前等一下沒出現就算失敗，現在會重新載入頁面再試一輪。另外，用加速模式的人以前等待時間被縮短到不到五秒，失敗會比別人多，這版統一了。
* **回報問題的送出按鈕不會被擠出畫面**：以前訊息長一點就要縮小瀏覽器才按得到。
* **診斷紀錄整理過**：你回報問題時附上的技術資訊，現在更容易看出是哪一步卡住。
