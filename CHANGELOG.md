> 判斷發布狀態請以 `gh release list` 或商店實際上架版本為準，不要以
> commit 訊息裡的 `release:` 字樣為憑（那只代表當時打包過）。
> `v2.8.4-betaN` 條目是開發過程紀錄，全部已併入 v2.8.4 正式版。

## v2.8.4-beta37 — 修掉「命名可疑」誤標真人，標籤強度依實際判定分流

* **TL;DR：**beta33 為了讓貼文卡片也能標記，用帳號名長相初判英文名鏡像。實機立刻誤標真人：`sweetrice039220412`（顯示名「吳佳玲」，2.1 萬粉絲）與網軍帳號的形狀完全相同，光看帳號名區分不出來。形狀初判收回。
* **改成兩條有依據的路：**掃描結果已存有判定旗標的直接沿用；正停在該帳號個人頁時，從網頁標題取得顯示名做原本的雙重比對。兩條都拿得到顯示名，中文顯示名不可能誤中。動物字典與拼音只需要帳號名，不受影響。
* **順帶修正標籤強度：**待審清單同時收「真三無」與「命名命中但仍有活動」兩種，過去一律標成「疑似假帳號」，誇大了結論。現在只有真三無掛強標籤，命名命中的掛「命名可疑」提示。舊資料沒有該欄位時維持原行為。

## v2.8.4-beta36 — beta 診斷直送本機，實機問題不再靠手動複製定位

* **TL;DR：**beta 版每 4 秒把診斷環自動送到開發端本機的接收程式（內容沒變就不送）。過去這類「只在實機出現」的問題，得請使用者一次次貼 console 指令來回定位，光是勾選框消失就繞了五輪。
* **邊界：**只送到 `127.0.0.1`，不出本機網路；只在 beta 生效，正式版 `ENABLE_BETA_DIAGNOSTICS` 為 false 時完全不會有任何連線。推送內容沿用既有白名單，只有數量與布林值，不含帳號、選單文字或網址。
* **失敗安全：**接收端沒啟動時 fetch 失敗即靜默略過，不影響封鎖、掃描或任何產品行為。

## v2.8.4-beta35 — 個人頁往下滑不再整排掉勾選框（真正的根因）

* **TL;DR：**在某個帳號的個人頁上，每則貼文的作者都等於頁面主人，程式過去靠「按鈕在畫面上緣以內」來區分頁面標題區和貼文。這是捲動就會失效的判準：往下滑時貼文位置變小甚至變成負數，於是每一則捲過頂端的貼文都被誤判成標題區，蓋上永久跳過的印記，之後再也不處理。實測門檻 330，位置 -370／-204／272 的三則全數漏掉，380／856 的兩則正常，零例外。
* **修法：**兩處座標判定全部移除（一處攔注入、一處刪除已注入的勾選框），改由既有的貼文情境判定負責。該判定看的是結構（貼文一定有作者名與時間，標題區沒有），實測對標題區與粉絲數列判否、對四則貼文判過，本來就分得清楚。
* **迴歸防線：**新增測試釘住，座標式標題區判定不得再被引入。
* **附帶：**新增 `scripts/inspect-live-page.mjs`，開發端可直接連上使用者的 Chrome 讀取實機 DOM 狀態，這類只在實機出現的問題不必再靠手動貼 console 指令來回定位。

## v2.8.4-beta34 — 修好往下滑勾選框與標籤消失，三條注入路徑收成一條

* **TL;DR：**Threads 的貼文列表會回收 DOM 節點重複利用。舊的「已處理」判斷有兩個洞：一是只要父層底下有任何一個勾選框就跳過，同一區塊的第二則貼文因此永遠裝不上；二是「已處理」印記在勾選框真的掛上去之前就蓋，掛設失敗或隨即被清掉時印記還留著，那則貼文就此永久被跳過。
* **修法：**改成認「這則貼文、這個帳號、而且勾選框確實還連在正確位置」才算已處理；印記改成掛設成功並確認還在畫面上才蓋，任何失敗路徑都會清掉印記。
* **注入路徑統一：**貼文那條路原本自己刻圖示、自己綁事件，跟共用工廠函式是逐字複本。改為統一呼叫工廠，刪掉四十行複本，事件種類、順序、capture 旗標與 SVG 屬性逐項比對一致，零行為變化。
* **加了診斷計數：**每個「跳過不裝勾選框」的關卡都有計數（對話框、圖示過濾、找不到帳號、自己的帳號、個人頁頂部、非貼文情境、已有有效勾選框、父層失效、掛設失敗）。只有數字與布林，不含帳號名。下次若還有漏，匯出診斷即可直接定位，不必再猜。

## v2.8.4-beta33 — 貼文與個人頁的「命名可疑」標籤補上拼音與英文鏡像

* **TL;DR：**beta32 的新判定只用在三無掃描的待審清單，貼文和個人頁上的即時標籤還是只認得動物字典，所以直接開假帳號的頁面看不到任何提示。判斷式補上拼音（ADR 0024）與英文名鏡像（ADR 0028）兩種訊號。
* **限制說明：**貼文卡片只呈現帳號名、沒有顯示名，英文鏡像在此只能用帳號名長相初判，比掃描時的顯示名雙重比對寬鬆，`johnsmith12345` 這類真人命名也會被標。標籤僅提示不封鎖，先取寬鬆側。

## v2.8.4-beta32 — 抓新形態網軍：顯示名鏡像帳號名

* **TL;DR：**網軍批次註冊改用新命名法，帳號名開頭就是顯示名去空格的小寫形式，後面接兩到四個亂碼字母加五碼數字（顯示名 `Kenneth Berry` 對應帳號 `kennethberryaei31413`）。這批帳號都有盜來的頭像，舊的動物字典和無頭像門檻完全抓不到。（ADR 0028）
* **判定與拼音對稱：**命中且內容確認為空就計入三無；命中但有活動的不算三無，改進待審清單掛「英文名鏡像・疑似批次註冊」標籤。私密帳號依舊不會經此路徑進三無。維持人工逐筆確認，不自動封鎖。
* **誤判防護：**要求顯示名與帳號名雙重吻合，真人的 `johnsmith12345`（沒有亂碼中綴）、中文顯示名、顯示名與帳號名對不上的一律排除，測試已釘住。
* **效能：**只有帳號名長相符合的少數帳號才會抽取顯示名比對，兩千人以上名單不會逐列付出額外成本。

## v2.8.4-beta31 — 堵掉兩個「假成功」破口，重新驗證不再被殭屍狀態卡死

* **TL;DR：**對抗性審查抓到 beta28/29 留下的破口：確認視窗被限流警告換裝時會被誤判成封鎖成功。修正後一律先查限流再判成功；複驗也收緊，「頁上沒視窗」必須加上「當初那個確認視窗確實卸載」才算數。（PLAN #34）
* **重新驗證解鎖：**worker 中途被關留下的殘留狀態，過去會讓「重新驗證」按鈕被「請先停止」永久擋住。現在 worker 沒在跑就自動清掉殘留再啟動。（PLAN #35a）
* **測試：**限流換裝時序（先掛限流框、再卸確認框）有專屬測試釘住，不會再回歸。

## v2.8.4-beta30 — 診斷去重盤點釘住與保留窗常數收斂

* **TL;DR：**盤點全部診斷簽章去重呼叫點，確認漂移計數（選單數、候選數、列數）都已排除在簽章外並補回歸測試釘住，閒置時不再狂寫重複紀錄洗掉關鍵證據；48 小時保留窗收斂成單一常數。（PLAN #25、#14）
* **行為不變：**量測值仍照常記錄在診斷欄位裡，只是不參與「有沒有變化」的判斷；真實狀態變化（找到、消失、位移、注入）仍必定記錄。

## v2.8.4-beta29 — 封鎖確認改盯著那扇視窗等，背景分頁不再漏看

* **TL;DR：**點下「確認封鎖」後，改成追蹤那個確認視窗本身有沒有被移除，用 MutationObserver 即時收通知；不再等「全頁視窗歸零」，也不再被背景分頁的計時器降速拖累。（ADR 0027）
* **等待預算：**總超時 15 秒，不受速度模式縮放；超時仍照走限流檢查、頁面文字檢查與 beta28 複驗，判定鏈一層不少。
* **診斷：**確認成功的紀錄新增 observer 標記，日後回報可分辨新舊偵測機制。

## v2.8.4-beta28 — 封鎖成功不再被誤列失敗，失敗名單可一鍵重新驗證

* **TL;DR：**修正線上回報 #53／#54：封鎖其實成功、卻被列進失敗名單顯示「動作失敗」。根因是確認後只等 5 秒看視窗關閉，Threads 變慢時判定比實際完成更早。（ADR 0026）
* **失敗前複驗：**即將判定失敗時先固定等 3 秒再看一次，視窗已關或頁面已顯示解除封鎖就改判成功。
* **重新驗證：**失敗清單新增「重新驗證」按鈕，逐帳號檢查實際封鎖狀態，已封鎖者自動移出名單並補進封鎖資料庫；帳號已消失時最多導頁重試一次，不會卡住。

## v2.8.4-beta27 — 拼音命名帳號一律進待審清單，活躍者掛琥珀標籤

* **TL;DR：**拼音命名本身視為可疑訊號，有發文也不豁免；活躍者卡片掛「拼音命名・有活動」琥珀標籤與三無帳號區分。（ADR 0025）
* **統計口徑：**三無公式不動，統計只數真三無；名單改稱待審清單，顯示「待審 N 筆，其中三無 M 筆」。維持人工逐筆確認，無自動封鎖。

## v2.8.4-beta26 — 有頭像的拼音空帳號不再隱形

* **TL;DR：**實機案例證明有頭像的簡體拼音空帳號會被頭像門檻整個跳過，個體層級完全不可見。新增第二條入口：拼音命名且至少一項內容確認為空，即可進待審清單。（ADR 0024）
* **邊界：**私密帳號不經此路徑；拼音命中結果持久化到 finding 供 UI 顯示。

## v2.8.4-beta25 — 動物數字門檻改回 5 位

* **TL;DR：**beta24 收到 6 位後實測漏抓 shark20819 這類 Threads 預設 5 位帳號，拍板改 5 位；4 位仍排除，避開真人生日式自取名。

## v2.8.4-beta24 — 拼音判定收窄成對岸獨有音節

* **TL;DR：**拼音命中改成必須含漢語拼音獨有音節（zh/x/q/z/c 開頭），台灣常用的 Chang／Hsieh／Tsai 等威妥瑪拼法不再誤中；chen／lin／wang 這類兩岸通用音節也不再單獨算數。
* **新標籤：**「疑似簡體拼音」資訊標籤純顯示，不計分不判定；動物數字門檻本版暫收 6 位（beta25 調回 5 位）。

## v2.8.4-beta23 — 名冊連續同構命名密度警報

* **TL;DR：**實機名冊顯示正常追蹤者的拼音姓名結構最長連續 2 個，網軍批次註冊會連排十幾個。連續命中達 5 個時在三無報告頂端掛資訊橫幅提示疑似批次註冊，純提醒，不判定不封鎖。

## v2.8.4-beta22 — 結算配額防爆與重置備份清理

* **TL;DR：**修掉 2,760 人大掃描在結算時撞 localStorage 5MB 配額、終態寫不出去導致無聲卡死的問題。（ADR 0023）
* **三級降級：**結果寫入依序丟棄 debugLog、縮減 metadata、最後保底只寫終態標記，保證掃描一定有結果狀態。
* **配額元凶：**六月重置留下的備份佔 1.44M 且從無清理，改為最多留最新一份、逾七天自動清。

## v2.8.4-beta21 — 可疑命名動物字典補 11 個缺字

* **TL;DR：**實機名冊比對補上 gecko、yak、raccoon 等 11 個缺字。此規則全鏈路僅提醒，不自動封鎖。

## v2.8.4-beta20 — 三無公式改為必須有確認過的無內容證據

* **TL;DR：**62 筆實機取證證明誤判全部來自公式：沒簡介或私密帳號配上無頭像就成立，把活躍真人判成三無。新公式必須至少一項內容證據（無貼文／無回覆／無轉發／可疑命名）確認成立才判。（ADR 0022）
* **重算結果：**62 筆保留 33、排除 29，誤判樣本全數排除。

## v2.8.4-beta19 — 三無取證補追蹤中數量與兩個命名訊號

* **TL;DR：**三無 beta 名冊沿用已開啟的個人頁 probe，記錄粉絲數、追蹤中數量與各自是否成功取得；追蹤中找不到時保留明確原因與嘗試樣式，不影響三無判定。
* **命名取證：**名冊新增顯示名結尾重複姓氏與阿拉伯／波斯文字兩個布林訊號，只使用既有帳號名與顯示名，不參與候選、三無判定或既有可疑命名旗標。
* **隱私與成本：**實際開啟 profile 的列才寫入 count；未開啟列明確標成未取證。名冊仍只留本機，不進平台 payload、RuntimeDiagnostics ring 或問題回報附件；storage 寫入維持 beta18 固定批次水準。
...
* **空白第一輪修正**：已確認為 Likes、等待版面後仍為 0 列且沒有 unknown／truncated 證據時，會繼續探測第二排序。
* **真實計數標題修正**：Threads 的 `1,744個讚` 目前可能是一般元素而不是 heading；清理名單現在沿用對話框標題解析，不會把這種 Likes 視窗誤當成一般名單而跳過第二輪。
* **重新載入等待**：切換第二種排序後，優先觀察目前 Likes 視窗內的 `載入中……` 轉圈圈；即使 React 暫留 1 列舊資料，轉圈還在也不會被 5 秒無進度期限提前結束。等待最長 30 秒，之後仍回到原本的最後進度停止條件。
* **大型名單提醒**：收集達 200 人時，進度區會提醒 Threads 網頁可能開始變慢；若操作已明顯變慢，可按「停止並結算」保留目前名單，之後再繼續掃描。提醒不會自動中止掃描。
* **仍維持 fail-closed**：空白第一輪不會自己被當成成功；沒有排序控制、切換失敗或第二輪不完整時，整次仍不提交名單。
* **回歸契約**：production-path fixture 讓 dialog loading 持續 20 秒，驗證空白與暫留 1 列兩種情況都會等待並取得第二輪帳號；no-sort fixture 驗證空白結果仍不可提交。

## v2.8.4 — 封鎖不再誤報失敗、勾選框不再消失、假帳號判定整輪重寫（2026-08-16）

* **TL;DR：這版修掉三個最痛的問題：封鎖明明成功卻被列進失敗名單、個人頁往下滑勾選框整排消失、假帳號掃描把活躍真人誤判成三無。另外抓得到新形態的網軍命名了。**

**封鎖與失敗名單**

* **封鎖成功不再被誤列失敗**：以前確認封鎖後只等 5 秒看視窗有沒有關，Threads 變慢時判定比實際完成更早，成功也被記成失敗。現在改成盯著那扇確認視窗本身等它關閉，即時收通知，切到背景分頁也不會漏看；即將判失敗前還會再複驗一次。
* **失敗名單可一鍵重新驗證**：新增「重新驗證」按鈕，逐帳號檢查實際封鎖狀態，其實已封鎖的自動移出名單並補進封鎖資料庫。之前 worker 中途被關導致按鈕被「請先停止」永久擋住的問題也一併修掉。
* **限流不再被當成成功**：確認視窗被限流警告換裝時，以前可能誤判成封鎖成功，現在一律先查限流再判。

**勾選框與標籤**

* **個人頁往下滑不再整排掉勾選框**：舊版用「按鈕位置在畫面上緣以內」區分標題區和貼文，捲動時位置變化就整批誤判並永久跳過。座標判定整個移除，改看貼文結構本身，滑多遠都不會掉。
* **列表回收不再吃掉勾選框**：Threads 會回收畫面元件重複利用，舊的「已處理」判斷會把同區塊第二則貼文或回收後的貼文永久跳過，已修正。

**假帳號（三無）掃描**

* **不再把活躍真人判成三無**：判定公式改為必須至少一項內容證據（無貼文／無回覆／無轉發／可疑命名）確認成立才算，62 筆實機樣本中 29 筆誤判全數排除。
* **抓得到新形態網軍**：批次註冊改用「顯示名鏡像帳號名」命名法（顯示名 Kenneth Berry 對應帳號 kennethberryaei31413），這批帳號有頭像、舊規則完全抓不到，現在會進待審清單。拼音命名帳號也一律進待審清單。
* **標籤分得清楚**：真三無標「疑似假帳號」，只是命名可疑但仍有活動的標琥珀色提示，不再混用；也修掉把中文顯示名真人誤標「命名可疑」的問題。所有判定僅提示，維持人工逐筆確認，不自動封鎖。
* **兩千人大掃描不再卡死**：大量掃描在結算時撞到瀏覽器儲存空間上限會無聲卡住，現在會自動降級寫入，保證掃描一定有結果。

**清理名單與其他**

* **清理名單以前會漏人**：按讚名單只掃一種排序，沒掃到的就當作不存在。現在兩種排序都掃過再合併，重複的自動去掉；第一輪空白也會繼續掃第二排序。
* **按停止不再等於整批取消**：「停止並結算」會把已抓到的留給你；三無掃描按停止也會保留進度並打開待審清單。
* **名單很大時不會越跑越慢**：收集期間先停掉背景掃描，結束後補一次就好；達 200 人時提醒網頁可能變慢，但不會自動中止。
* **每日上限的數字以前算錯**：只有真的封鎖成功才算一筆，且每筆滿 24 小時自動退掉，提醒不會再太早跳出來。
* **封鎖遇到頁面沒載出來會再試一次**：會重新載入頁面再試一輪；加速模式的等待時間也統一了。
* **回報問題的視窗有捲軸了**：訊息長一點也按得到送出按鈕，不用縮小瀏覽器。

## v2.8.3 — 檢舉載入等待、勾選掃描效能與診斷改善

* **TL;DR：檢舉會等頁面載入完成，失敗原因更準確，勾選框掃描不再逐列重建封鎖名單；問題回報保留能查出根因的輕量診斷。**
* **檢舉流程**：檢舉會等待目標帳號頁面載入完成，不再只等待數毫秒就放棄；失敗時不再誤報「找不到選單」。
* **勾選框掃描**：逐列掃描沿用同一輪狀態快照，不再逐列重建封鎖名單，大量列時不會拖慢整個分頁。
* **診斷改善**：收斂重複紀錄並保留重要流程節點，讓問題回報更容易查出根因。

## v2.8.2 — 勾選框列判定收斂、冷卻保護停用、視窗尺量修正

* **TL;DR：修好查看動態各分頁的勾選框重複出現、位置錯誤與清單被撐開；取消自動冷卻保護改為只提醒；修正背景視窗撐不到可執行尺寸而卡住不跑的問題。**
* **勾選框列判定**：`injectDialogCheckboxes` 改由 `Core.resolveAccountRowContext` 統一判定帳號列、貼文卡、預覽卡、掛載節點與信心等級，淘汰兩條寫死的 Threads class 規則，引用與轉發不再各自維護分頁特例。
* **安全列界線**：往上尋找列容器時加入高度守衛，拒絕數千像素的共用祖先；拿不到可信列時退回「這輪跳過、下輪重試」，並保留巢狀 dialog 的有界低信心救援。低信心框會在取得更高信心列後自動搬移。
* **版面對齊**：貼文卡與帳號列的框統一對齊列尾端，ID 長度不再改變水平位置；緊湊預覽卡依可用寬度自動選用較小尺寸，確保框完整落在列內且不與既有元素或下一列重疊。
* **預覽卡只留發文者**：預覽卡在卡片層級只保留發文者的框，被引用者、被回覆者與內文提及不再產生框；判定不出預覽卡時退回一般帳號列處理，不會讓整份清單少長框。
* **冷卻保護停用**：連續驗證失敗、選單異常或平台限制訊號不再自動進入 12 小時冷卻、不搬移名單、不回滾資料庫；改為分級提醒，達三次才建議手動暫停。既有冷卻備份可從主面板併回待處理清單。
* **每日上限提醒**：worker 縮小模式與主面板都會醒目顯示已封鎖數與自訂安全估計值；提醒只提供風險資訊，流程仍會依使用者設定繼續執行。
* **視窗尺量**：worker 視窗改以內容區缺口補足尺寸，移除會與暫停閘門打架的外框上下界，並補上 resize 診斷取證。

## v2.8.1 — 修正 2.8.0 的封鎖失敗，並讓問題回報查得出根因

* **TL;DR：2.8.1 修正 2.8.0 之後的封鎖失敗，並一併修好視窗縮小、檢舉點錯選單、重試失敗清單與清理名單等問題。**
* **2.8.0 全面封鎖失敗**：2.8.0 為了避免點到錯的人，加了一道「先確認這是不是目標對象的個人頁」的檢查，但那道檢查把帳號名稱的文字比對與版面位置都做成一票否決，任一條在真實版面對不上就整批失敗，而且沒有任何退路。現在改為嚴格判定先跑，失敗時在網址已確認停在本人個人頁的前提下改用寬鬆判定，身分證明保留，容錯拉回來。
* **視窗縮小時連續失敗**：工作視窗被縮到很小時，個人頁的功能按鈕被擠出可視範圍而找不到。現在視窗過小會暫停等待，不取號也不記失敗，放大後自動繼續。
* **檢舉點到左側主選單**：檢舉視窗還沒開啟時，程式會退回整頁尋找選項，因而點到左側導覽列。現在沒有檢舉視窗就停手。
* **重試失敗清單後清單被清空**：失敗紀錄原本在重試開始前就先清掉，重試再失敗就什麼都不剩。現在要等真的成功才逐筆移除。
* **清理名單過早判定失敗**：高按讚數貼文的名單還在排版就被判定為空。現在會等名單真的排出來才判定。
* **問題回報附帶技術資訊**：先前正式版完全不收集診斷，使用者回報功能故障時無法判斷原因。現在問題回報會一併附上工具版本、瀏覽器環境、視窗尺寸與最近的操作步驟代號；這部分不含帳號名稱、選單文字、頁面網址與瀏覽紀錄，並在送出前的畫面上直接說明。完整診斷附件仍需逐次勾選同意。
* **勾選框閃爍**：封鎖啟動後帳號會從待處理名單移到背景佇列，但只有部分程式路徑認得背景佇列，另一部分會把勾選拿掉，兩者以不同頻率輪流執行。現在所有位置共用同一份狀態推導，勾選在整個封鎖過程維持穩定。
* **移除中間狀態**：背景佇列中的帳號原本會套用一個沒有對應樣式的中間狀態，畫面上與未勾選完全相同；該狀態已移除。
* **封鎖失敗**：程式要先認出目前這頁是誰的個人頁，才會去點「⋯」開始封鎖。認的條件是同時找到帳號名稱與頁面上那排功能按鈕的其中一個，缺一就判定不是個人頁並整個放棄。負責找功能按鈕的是一份自己重寫的簡化判定，只認圖示形狀與文字，沒有檢查按鈕是否真的可見、是否在正確區塊、也不會在多個候選之間評分，版型稍有變化就找不到，導致封鎖流程還沒開始就失敗。現在改用實際點按鈕時所用的同一套嚴謹判定；Instagram 與通知鈴鐺兩種按鈕的既有優先序不變，搜尋與主題標籤頁的保護範圍反而更嚴。
* **匯入名單重複封鎖**：貼上的帳號原本保留大小寫，而頁面抓到的帳號一律小寫，導致已封鎖過的帳號被當成新目標重新排隊、重新封鎖，同一個人在紀錄裡留下兩筆，該帳號的勾選框也永遠不顯示已完成。分享連結網址後面的井號片段原本也會被當成帳號名的一部分，產生不存在的帳號而必然失敗。匯入現在一律折成小寫並截掉網址的查詢字串與井號；既有紀錄的大小寫差異在比對時一併吸收，舊資料不改動。
* **測試**：新增封鎖進行中連續多輪重繪的勾選穩定性回歸測試。既有 beta 期診斷測試改為依產品閘門條件式跳過，正式版不再出現與功能無關的測試失敗，測試版會自動恢復執行。
