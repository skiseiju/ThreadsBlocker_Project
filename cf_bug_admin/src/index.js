import {
  extractTopicTerms as extractAmplificationTopicTerms,
  normalizeTopicHints as normalizeAmplificationTopicHints,
  normalizeTopicLabel as normalizeAmplificationTopicLabel,
  normalizeTopicRows
...
var PUBLIC_COORDINATION_HIGH_OBSERVER_MIN = 3;
var PUBLIC_COORDINATION_WATCH_OBSERVER_MIN = 2;
var PUBLIC_COORDINATION_MAX_CONCENTRATION = 60;
var PUBLIC_COORDINATION_HIGH_SCORE = 60;
...
var SAMPLE_REVIEW_STATUSES = ["pending", "approved", "rejected"];
...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      if (url.pathname === "/" && request.method === "GET") {
        return withCors(json({
          ...
          endpoints: {
            ...
            adminSampleReviews: "GET /api/v1/admin/sample-reviews?status=pending (Bearer token)"
          }
        }, 200));
      }
      ...
      if (url.pathname === "/api/v1/admin/sample-reviews" && request.method === "GET") {
        return withCors(await handleAdminSampleReviews(request, env));
      }
      const sampleReviewAction = url.pathname.match(/^\/api\/v1\/admin\/sample-reviews\/(\d+)\/(approve|reject)$/);
      if (sampleReviewAction && request.method === "POST") {
        return withCors(await handleAdminSampleReviewDecision(request, env, sampleReviewAction));
      }
...
async function handleAdminSampleReviews(request, env) {
  ...
  if (!SAMPLE_REVIEW_STATUSES.includes(requestedStatus)) {
    return json({ code: 400, message: "Invalid sample review status" }, 400);
  }
  const limit = clampInt(url.searchParams.get("limit"), 1, 500, 100);
  const rows = await env.DB.prepare(
    `SELECT id, topic_id, sample_text, deidentified_text, account_count, observer_count,
            status, created_at, decided_at
       FROM topic_sample_reviews
      WHERE status = ?
      ORDER BY id DESC
      LIMIT ?`
...
async function handleAdminSampleReviewDecision(request, env, match) {
  ...
  if (!Number.isFinite(id) || id <= 0 || !["approve", "reject"].includes(action)) {
    return json({ code: 400, message: "Invalid sample review action" }, 400);
  }
  const existing = await env.DB.prepare(
    "SELECT id FROM topic_sample_reviews WHERE id = ?"
  ).bind(id).first();
  if (!existing) return json({ code: 404, message: "Sample review not found" }, 404);
  const status = action === "approve" ? "approved" : "rejected";
  const decidedAt = (/* @__PURE__ */ new Date()).toISOString();
  await env.DB.prepare(
    "UPDATE topic_sample_reviews SET status = ?, decided_at = ? WHERE id = ?"
  ).bind(status, decidedAt, id).run();
  return json({ code: 200, data: { id, status, decided_at: decidedAt } }, 200);
}
__name(handleAdminSampleReviewDecision, "handleAdminSampleReviewDecision");
...
async function loadPlatformOverviewData(env, days, top, options = {}) {
  ...
  const recentPoliticalEventRows = await env.DB.prepare(
    `SELECT event_date, category, title, source_name
      ...
      WHERE date(event_date) >= date('now', '-${AUTO_EVENT_LOOKBACK_DAYS} days')
      ORDER BY event_date DESC, id DESC
      LIMIT 200`
  ).all();
  const recentPoliticalEvents = (recentPoliticalEventRows.results || []).map((row) => ({
    event_date: safeString(row?.event_date || "", 20),
    category: safeString(row?.category || "", 80),
    title: safeString(row?.title || "", 200),
    source_name: safeString(row?.source_name || "", 100)
  })).filter((row) => row.event_date && row.title);
  const dynamicEventCandidates = buildDynamicEventCandidates(recentPoliticalEvents);
  ...
  const narrativeRows = await env.DB.prepare(
    `SELECT
      ...
      MAX(top_topic_hints_json) AS top_topic_hints_json
...
async function loadApprovedTopicSampleReviews(env) {
  const rows = await env.DB.prepare(
    `SELECT id, topic_id, deidentified_text, account_count, observer_count, status, decided_at
       FROM topic_sample_reviews
...
__name(loadApprovedTopicSampleReviews, "loadApprovedTopicSampleReviews");
async function queueEligibleTopicSamples(env, sampleRows, topicDefinitions) {
  // 輸入兩種形狀皆可：
  // (a) 專屬 SQL 已按原文聚合的列（sample_text / observer_count / account_count）
  // (b) 舊 per-source 列（source_text_sample / source_url / unique_account_count）
  // 兩者都在此以「去識別化後文字」再合併一次，讓只差 @帳號名的變形洗版歸為同一樣本
  const buckets = /* @__PURE__ */ new Map();
  for (const row of Array.isArray(sampleRows) ? sampleRows : []) {
    const sampleText = safeString(row?.sample_text || row?.source_text_sample || "", 320);
    if (!sampleText) continue;
    const deidentifiedText = deidentifySampleText(sampleText);
    if (!deidentifiedText) continue;
    const matchedTopicIds = (Array.isArray(topicDefinitions) ? topicDefinitions : [])
      .filter((definition) => safeString(definition?.id || "", 80) && topicDefinitionMatchesText(definition, sampleText))
      .map((definition) => safeString(definition.id, 80));
    // 沒命中任何話題的重複文字也要進佇列（大量重複的話術常落在策展話題之外），歸入 general
    const targetTopicIds = matchedTopicIds.length ? matchedTopicIds : ["general"];
    for (const topicId of targetTopicIds) {
      const key = `${topicId}::${deidentifiedText}`;
      const bucket = buckets.get(key) || {
        topicId,
        sampleText,
        deidentifiedText,
        accountCount: 0,
        aggregatedObservers: 0,
        observerUrls: /* @__PURE__ */ new Set()
      };
      bucket.accountCount += safeCount(row?.account_count || row?.unique_account_count);
      const preAggregated = safeCount(row?.observer_count);
      if (preAggregated > 0) {
        // 聚合列相加為近似值：同一來源若發過兩種原文變形會被重複計，屬可接受的高估下限保守面
        bucket.aggregatedObservers += preAggregated;
      } else {
        const sourceUrl = safeString(row?.source_url || "", 500);
        if (sourceUrl) bucket.observerUrls.add(sourceUrl);
      }
...
__name(queueEligibleTopicSamples, "queueEligibleTopicSamples");
function topicDefinitionMatchesText(definition, text) {
  const value = safeString(text || "", 2e3).toLowerCase();
...
function deidentifySampleText(input) {
  // Person-name detection is intentionally not automatic; human review owns that decision.
  return compactWhitespace(String(input || "").slice(0, 2e3))
    .replace(/\b(?:https?:\/\/|www\.)[^\s<>"'，。！？!?、；;：:)\]}]+/giu, "[連結]")
    .replace(/(^|[^\p{L}\p{N}_])@[a-zA-Z0-9._-]{1,64}/gu, "$1[帳號]")
    .replace(/回覆\s*[a-zA-Z0-9._-]{2,64}/gu, "回覆[帳號]")
    .replace(/\b[a-zA-Z][a-zA-Z0-9]*(?:[._][a-zA-Z0-9._]+)+\b/g, "[帳號]")
    .replace(/\b[a-zA-Z]+[0-9][a-zA-Z0-9._-]*\b/g, "[帳號]")
    .replace(/\b(?:\+?886[-\s]?|0)(?:9\d{2}[-\s]?\d{3}[-\s]?\d{3}|[2-8]\d?[-\s]?\d{3,4}[-\s]?\d{3,4})\b/g, "[電話]");
}
__name(deidentifySampleText, "deidentifySampleText");
function buildDynamicEventCandidates(events, staticDefs = PUBLIC_CANDIDATE_TOPIC_DEFS, referenceDate) {
  const staticKeywordSet = new Set((Array.isArray(staticDefs) ? staticDefs : []).flatMap((def) => Array.isArray(def?.keywords) ? def.keywords : []).map((term) => safeString(term, 80).toLowerCase()));
  const seenKeywords = /* @__PURE__ */ new Set();
  const candidates = [];
  for (const [index, rawEvent] of (Array.isArray(events) ? events : []).entries()) {
    const event = rawEvent && typeof rawEvent === "object" ? rawEvent : {};
    const eventDate = parseDateKey(event.event_date || event.eventDate || "");
    if (!eventDate || !isWithinLookback(eventDate, referenceDate, AUTO_EVENT_LOOKBACK_DAYS)) continue;
    const title = safeString(event.title || "", 200);
    const terms = extractAmplificationTopicTerms(title).map((term) => safeString(term, 80)).filter(Boolean);
    const freshTerms = terms.filter((term) => !staticKeywordSet.has(term.toLowerCase()));
    if (!freshTerms.length || freshTerms.some((term) => seenKeywords.has(term.toLowerCase()))) continue;
    for (const term of freshTerms) seenKeywords.add(term.toLowerCase());
    const slug = freshTerms.join("-").replace(/[^\p{L}\p{N}.-]+/gu, "-").replace(/^-+|-+$/g, "").slice(0, 60) || String(index + 1);
    candidates.push({
      id: `auto-event-${slug}`,
      kind: "auto_event",
      strength: "candidate",
      title,
      dateHint: eventDate,
      keywords: freshTerms.slice(0, 8),
      note: "由最近 14 日政治事件標題自動抽取的錨點，僅作為話題對照。",
      eventAnchorCount: 1
    });
  }
  return candidates;
}
__name(buildDynamicEventCandidates, "buildDynamicEventCandidates");
...
function isWithinLookback(eventDate, referenceDate, lookbackDays) {
  const eventKey = parseDateKey(eventDate);
  const referenceKey = parseDateKey(referenceDate) || parseDateKey(new Date());
  if (!eventKey || !referenceKey) return false;
  const eventMs = Date.parse(`${eventKey}T00:00:00Z`);
  const referenceMs = Date.parse(`${referenceKey}T00:00:00Z`);
  const diff = referenceMs - eventMs;
...
function projectPublicPlatformOverview(raw, top = 15, options = {}) {
  ...
  const approvedSampleReviews = Array.isArray(data.approvedSampleReviews) ? data.approvedSampleReviews : [];
  const dynamicEventCandidates = Array.isArray(data.dynamicEventCandidates) ? data.dynamicEventCandidates : buildDynamicEventCandidates(
    Array.isArray(data.recentPoliticalEvents) ? data.recentPoliticalEvents : [],
    PUBLIC_CANDIDATE_TOPIC_DEFS,
    data.referenceDate
  );
  ...
  const candidateTopics = buildPublicCandidateTopics({
    topicRows: normalizedTopicRows,
    topicTimeSeries: normalizedCandidateTopicDaily,
    sourceRows,
    dynamicEventCandidates,
    referenceDate: data.referenceDate
  });
  const topicCards = buildPublicTopicCards({
    topicRows: normalizedTopicRows,
    topicTimeSeries: normalizedTopicTimeSeries,
    narratives,
    candidateTopics,
    approvedSampleReviews,
  ...
  return {
    ...
    thresholds: {
      ...
      coordinationHighObserverMin: PUBLIC_COORDINATION_HIGH_OBSERVER_MIN,
      coordinationWatchObserverMin: PUBLIC_COORDINATION_WATCH_OBSERVER_MIN,
    ...
    methodology: {
      ...
      principles: [
        ...
        "\u6BCF\u8A71\u984C\u5354\u8ABF\u5206\u6578\u4F86\u81EA\u64F4\u5145\u7AEF per-source \u8A0A\u865F\u805A\u5408\u3001\u4F86\u6E90\u96C6\u4E2D\u8207\u6642\u9593\u96C6\u4E2D\uFF0C\u662F proxy\uFF0C\u4E0D\u662F\u91CD\u65B0\u63A8\u5C0E\u7684\u540C\u6587\u6BD4\u4F8B\uFF1B\u53EA\u6DB5\u84CB source_text_sample \u547D\u4E2D\u95DC\u9375\u5B57\u7684\u6D3B\u52D5\uFF0C\u4E0D\u6307\u8A8D\u8EAB\u5206\u3001\u4E0D\u5BA3\u7A31 AI\u3002"
      ]
    }
  };
}
__name(projectPublicPlatformOverview, "projectPublicPlatformOverview");
function buildPublicTopicTimeSeries(rows) {
  const byDate = /* @__PURE__ */ new Map();
  for (const item of Array.isArray(rows) ? rows : []) {
    const row = item && typeof item === "object" ? item : {};
    const date = safeString(row.date || row.day_key || "", 20);
    if (!date) continue;
    const topics = Array.isArray(row.topics) ? row.topics : [{ label: row.topic_label, count: row.event_count }];
    const bucket = byDate.get(date) || /* @__PURE__ */ new Map();
    for (const topic of topics) {
      const label = normalizeAmplificationTopicLabel(topic?.label || topic?.topic_label || "");
      if (!label) continue;
      bucket.set(label, (bucket.get(label) || 0) + safeCount(topic?.count || topic?.event_count));
    }
    if (bucket.size > 0) byDate.set(date, bucket);
  }
  return Array.from(byDate.entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([date, topics]) => ({
    date,
    topics: Array.from(topics.entries()).map(([label, count]) => ({
      label,
      count: safeCount(count)
    })).filter((topic) => topic.count > 0).sort((a, b) => safeCount(b.count) - safeCount(a.count)).slice(0, PUBLIC_CANDIDATE_TOPIC_LIMIT)
...
function buildPublicTopicCards({ topicRows, topicTimeSeries, narratives, candidateTopics, approvedSampleReviews, samplePublicationMode = "description", dateRange, top }) {
  ...
  const candidateCards = (Array.isArray(candidateTopics) ? candidateTopics : []).map((topic) => ({
    canonical_label: safeString(topic.title || topic.id || "", 120),
    title: safeString(topic.title || topic.id || "", 120),
    verdict: safeString(topic.coordinationBand || "\u89C0\u5BDF\u4E2D", 30),
    coordinationScore: round1(topic.coordinationScore),
    coordinationBand: safeString(topic.coordinationBand || "\u89C0\u5BDF\u4E2D", 30),
    coordinationSignal: topic.coordinationSignal || { weightedAverage: 0, max: 0 },
    crossObserverCount: safeCount(topic.crossObserverCount),
    sourceConcentrationPct: round1(topic.sourceConcentrationPct),
    peakConcentration: round1(topic.peakConcentration),
    ...
    dailySeries: Array.isArray(topic.topDays) ? topic.topDays.map((day) => ({
      ...
      eventCount: safeCount(day.eventCount)
    })).filter((day) => day.date && day.eventCount > 0).sort((a, b) => a.date.localeCompare(b.date)) : [],
    window: {
      start: safeString(topic.dateRange?.start || topic.displayDate || dateRange?.start || "", 20),
      end: safeString(topic.dateRange?.end || topic.displayDate || dateRange?.end || "", 20)
    },
    note: safeString(topic.note || "", 220),
    coordinationNote: safeString(topic.coordinationNote || "", 260)
  })).filter((card) => card.canonical_label);
  const candidateLabels = new Set(candidateCards.map((card) => card.canonical_label));
  const narrativeKeywords = new Set();
  for (const narrative of Array.isArray(narratives) ? narratives : []) {
    for (const keyword of Array.isArray(narrative.topKeywords) ? narrative.topKeywords : []) {
      const normalized = normalizeAmplificationTopicLabel(keyword);
      if (normalized) narrativeKeywords.add(normalized);
    }
  }
  const cards = [];
  for (const row of Array.isArray(topicRows) ? topicRows : []) {
    const label = normalizeAmplificationTopicLabel(row.topic_label || row.label || "");
    if (!label) continue;
    const dailySeries = series.map((entry) => {
      const count = (Array.isArray(entry.topics) ? entry.topics : []).filter((topic) => topic.label === label).reduce((sum, topic) => sum + safeCount(topic.count), 0);
      return { date: safeString(entry.date, 20), eventCount: count };
    }).filter((entry) => entry.date && entry.eventCount > 0);
    const eventCount = Math.max(safeCount(row.event_count || row.eventCount), dailySeries.reduce((sum, entry) => sum + safeCount(entry.eventCount), 0));
    ...
    if (safeCount(row.source_count || row.sourceCount) > 0) {
      evidence.push({
        type: "source_count",
        label: "\u73FE\u6709\u8CC7\u6599",
        value: safeCount(row.source_count || row.sourceCount),
        unit: "\u500B\u4F86\u6E90"
      });
    }
    if (narrativeKeywords.has(label)) {
      evidence.push({
        type: "narrative_keyword",
        label: "\u73FE\u6709\u8CC7\u6599",
        value: label,
        unit: "\u51FA\u73FE\u65BC\u91CD\u8907\u6558\u4E8B\u95DC\u9375\u5B57"
      });
    ...
    cards.push({
      ...
      dailySeries,
      window: {
        start: dailySeries[0]?.date || safeString(dateRange?.start || "", 20),
  ...
  const fallbackCards = cards.filter((card) => !candidateLabels.has(card.canonical_label)).sort((a, b) => {
    const aEvents = a.evidence.find((item) => item.type === "aggregated_events")?.value || 0;
    const bEvents = b.evidence.find((item) => item.type === "aggregated_events")?.value || 0;
    return safeCount(bEvents) - safeCount(aEvents);
  });
  return [...candidateCards, ...fallbackCards].slice(0, Math.max(1, safeCount(top)));
}
__name(buildPublicTopicCards, "buildPublicTopicCards");
function buildPublicTopicSamples(topicId, rows) {
  const id = safeString(topicId || "", 80);
  if (!id) return [];
  return (Array.isArray(rows) ? rows : []).filter((row) => {
    const status = safeString(row?.status || "", 20).toLowerCase();
    return safeString(row?.topic_id || "", 80) === id && status === "approved";
  }).map((row) => ({
    deidentified_text: safeString(row?.deidentified_text || "", 320),
    account_count: safeCount(row?.account_count),
...
__name(buildTopicPatternDescription, "buildTopicPatternDescription");
function removeBannedCopyTerms(text) {
  let result = String(text || "");
  for (const term of BANNED_COPY_TERMS) result = result.split(term).join("特定內容");
  return result;
}
__name(removeBannedCopyTerms, "removeBannedCopyTerms");
function buildPublicCandidateTopics({ topicRows, topicTimeSeries, sourceRows, dynamicEventCandidates, referenceDate }) {
  const topicList = Array.isArray(topicRows) ? topicRows : [];
  const series = Array.isArray(topicTimeSeries) ? topicTimeSeries : [];
  const sources = Array.isArray(sourceRows) ? sourceRows : [];
  const definitions = buildCandidateDefinitionList(dynamicEventCandidates);
  const candidates = definitions.map((def) => {
    ...
    const eventAnchorCount = safeCount(def.eventAnchorCount);
    ...
    return {
      ...
      coordinationBand: coordination.coordinationBand,
      coordinationSignal: coordination.coordinationSignal,
      crossObserverCount: coordination.crossObserverCount,
      sourceConcentrationPct: coordination.sourceConcentrationPct,
      peakConcentration: coordination.peakConcentration,
      activeDayCount: coordination.activeDayCount,
      timeConcentration: coordination.timeConcentration,
      coordinationNote: coordination.coordinationNote,
      recent7DayEventCount: calculateRecent7DayActivity(matchedDays, referenceDate),
      dateRange: {
        start: dayValues[0] || safeString(def.dateHint || "", 20),
        end: dayValues[dayValues.length - 1] || safeString(def.dateHint || "", 20)
      },
      topDays: matchedDays.slice().sort((a, b) => safeCount(b.eventCount) - safeCount(a.eventCount)).slice(0, 5)
    };
  }).filter((item) => item.eventCount > 0 || item.topicEventCount > 0 || item.sampleCount > 0 || item.eventAnchorCount > 0);
  const grouped = [];
  for (const kind of ["external_anchor", "community_native"]) {
    const limit = safeCount(PUBLIC_CANDIDATE_GROUP_LIMITS[kind]) || 6;
    grouped.push(...candidates.filter((item) => item.kind === kind).sort(compareCandidatePriority).slice(0, limit));
  }
  grouped.push(...candidates.filter((item) => item.kind !== "external_anchor" && item.kind !== "community_native"));
  return grouped.sort(compareCandidatePriority).slice(0, PUBLIC_CANDIDATE_TOPIC_LIMIT);
}
__name(buildPublicCandidateTopics, "buildPublicCandidateTopics");
function buildCandidateDefinitionList(dynamicEventCandidates) {
  const staticDefs = PUBLIC_CANDIDATE_TOPIC_DEFS.map((def) => ({ ...def }));
  const staticKeywordSet = new Set(staticDefs.flatMap((def) => Array.isArray(def.keywords) ? def.keywords : []).map((term) => String(term).toLowerCase()));
  const seenDynamicKeywords = /* @__PURE__ */ new Set();
  const dynamicDefs = [];
  for (const raw of Array.isArray(dynamicEventCandidates) ? dynamicEventCandidates : []) {
    const def = raw && typeof raw === "object" ? raw : {};
    const keywords = (Array.isArray(def.keywords) ? def.keywords : []).map((term) => safeString(term, 80)).filter(Boolean).filter((term) => !staticKeywordSet.has(term.toLowerCase()));
    if (!keywords.length || keywords.some((term) => seenDynamicKeywords.has(term.toLowerCase()))) continue;
    const id = safeString(def.id || "", 80);
    if (!id || staticDefs.some((item) => item.id === id) || dynamicDefs.some((item) => item.id === id)) continue;
    for (const keyword of keywords) seenDynamicKeywords.add(keyword.toLowerCase());
    dynamicDefs.push({ ...def, id, keywords });
  }
  return [...staticDefs, ...dynamicDefs];
}
__name(buildCandidateDefinitionList, "buildCandidateDefinitionList");
function compareCandidatePriority(a, b) {
  const bandRank = coordinationBandRank(b?.coordinationBand) - coordinationBandRank(a?.coordinationBand);
  if (bandRank !== 0) return bandRank;
  const recentActivity = safeCount(b?.recent7DayEventCount) - safeCount(a?.recent7DayEventCount);
  if (recentActivity !== 0) return recentActivity;
  const eventCount = safeCount(b?.eventCount) - safeCount(a?.eventCount);
  if (eventCount !== 0) return eventCount;
  return safeString(b?.displayDate || "", 20).localeCompare(safeString(a?.displayDate || "", 20));
}
__name(compareCandidatePriority, "compareCandidatePriority");
function coordinationBandRank(band) {
  if (band === "協調訊號偏高") return 3;
  if (band === "觀察中") return 2;
  if (band === "協調訊號低") return 1;
  return 0;
}
__name(coordinationBandRank, "coordinationBandRank");
function calculateRecent7DayActivity(days, referenceDate) {
  ...
  const cutoffMs = referenceMs - 6 * 24 * 60 * 60 * 1e3;
  return list.reduce((sum, day) => {
    const date = parseDateKey(day?.date || day?.day_key || "");
    if (!date) return sum;
    const dateMs = Date.parse(`${date}T00:00:00Z`);
    return dateMs >= cutoffMs && dateMs <= referenceMs ? sum + safeCount(day?.eventCount || day?.count) : sum;
  }, 0);
}
__name(calculateRecent7DayActivity, "calculateRecent7DayActivity");
function calculatePerTopicCoordination({ matchedSourceRows, matchedDays, eventCount }) {
  const sources = Array.isArray(matchedSourceRows) ? matchedSourceRows : [];
  const days = Array.isArray(matchedDays) ? matchedDays : [];
  let weightedSignalTotal = 0;
  let weightedEventTotal = 0;
  let maxSignal = 0;
  let maxSourceEvents = 0;
  const observers = /* @__PURE__ */ new Set();
  for (const row of sources) {
    const sourceUrl = safeString(row?.source_url || "", 500);
    if (sourceUrl) observers.add(sourceUrl);
    const sourceEvents = safeCount(row?.total_event_count || row?.eventCount);
    const signal = safeSignalScore(row?.avg_signal_score ?? row?.manipulation_signal_score);
    weightedSignalTotal += signal * sourceEvents;
    weightedEventTotal += sourceEvents;
    if (signal > maxSignal) maxSignal = signal;
    if (sourceEvents > maxSourceEvents) maxSourceEvents = sourceEvents;
  }
  const weightedAverage = weightedEventTotal > 0 ? round1(weightedSignalTotal / weightedEventTotal) : 0;
  const coordinationScore = weightedAverage;
  const denominator = Math.max(safeCount(eventCount), weightedEventTotal);
  const sourceConcentrationPct = denominator > 0 ? round1(maxSourceEvents / denominator * 100) : 0;
  const totalDayEvents = days.reduce((sum, day) => sum + safeCount(day?.eventCount || day?.count), 0);
  const peakDayEvents = days.reduce((max, day) => Math.max(max, safeCount(day?.eventCount || day?.count)), 0);
  const peakConcentration = totalDayEvents > 0 ? round1(peakDayEvents / totalDayEvents * 100) : 0;
  const activeDayCount = days.filter((day) => safeCount(day?.eventCount || day?.count) > 0).length;
  const crossObserverCount = observers.size;
  return {
    coordinationScore,
    coordinationBand: determineCoordinationBand(coordinationScore, crossObserverCount, sourceConcentrationPct),
    coordinationSignal: {
      weightedAverage,
      max: round1(maxSignal)
    },
    crossObserverCount,
    sourceConcentrationPct,
    peakConcentration,
    ...
    coordinationNote: "\u6B64\u5206\u6578\u4F86\u81EA\u64F4\u5145\u7AEF per-source \u8A0A\u865F\u805A\u5408\u3001\u4F86\u6E90\u96C6\u4E2D\u8207\u6642\u9593\u96C6\u4E2D\uFF0C\u662F proxy\uFF0C\u4E0D\u662F\u91CD\u65B0\u63A8\u5C0E\u7684\u540C\u6587\u6BD4\u4F8B\uFF1B\u53EA\u6DB5\u84CB source_text_sample \u547D\u4E2D\u95DC\u9375\u5B57\u7684\u6D3B\u52D5\uFF0C\u4E0D\u6307\u8A8D\u8EAB\u5206\u3001\u4E0D\u5BA3\u7A31 AI\u3002"
  };
}
__name(calculatePerTopicCoordination, "calculatePerTopicCoordination");
function determineCoordinationBand(score, crossObserverCount, sourceConcentrationPct) {
  const signal = safeSignalScore(score);
  const observers = safeCount(crossObserverCount);
  const concentration = safeCount(sourceConcentrationPct);
  if (
    signal >= PUBLIC_COORDINATION_HIGH_SCORE &&
    observers >= PUBLIC_COORDINATION_HIGH_OBSERVER_MIN &&
    concentration <= PUBLIC_COORDINATION_MAX_CONCENTRATION
  ) return "\u5354\u8ABF\u8A0A\u865F\u504F\u9AD8";
  if (signal < PUBLIC_MEDIUM_SIGNAL_THRESHOLD) return "\u5354\u8ABF\u8A0A\u865F\u4F4E";
  return "\u89C0\u5BDF\u4E2D";
}
__name(determineCoordinationBand, "determineCoordinationBand");
function buildCandidateTopicEvidence(topic) {
  const score = round1(topic?.coordinationScore);
  const observers = safeCount(topic?.crossObserverCount);
  const sourceConcentration = round1(topic?.sourceConcentrationPct);
  const peak = round1(topic?.peakConcentration);
  const activeDays = safeCount(topic?.activeDayCount);
  return [
    {
      ...
      value: score,
      unit: "\u5206\u5354\u8ABF proxy",
      text: `\u547D\u4E2D\u6B64\u8A71\u984C\u7684\u4F86\u6E90\uFF0C\u64F4\u5145\u7AEF\u5354\u8ABF\u8A0A\u865F\u52A0\u6B0A\u5E73\u5747\u70BA ${score} / 100`
    },
    {
    ...
    },
    {
      ...
      unit: "%\u6700\u5927\u55AE\u4E00\u4F86\u6E90\u5360\u6BD4",
      text: `\u6700\u5927\u55AE\u4E00\u4F86\u6E90\u4F54\u6B64\u8A71\u984C\u4E8B\u4EF6 ${sourceConcentration}%`
    },
    {
      ...
      value: peak,
      unit: "%\u5CF0\u503C\u65E5\u5360\u6BD4",
      text: `\u5CF0\u503C\u65E5\u4F54 ${peak}%\uFF0C\u6D3B\u8E8D ${activeDays} \u5929`
    }
  ];
}
__name(buildCandidateTopicEvidence, "buildCandidateTopicEvidence");
...
function buildPublicNarratives(rows, top, samplePublicationMode = "description") {
  ...
  for (const item of Array.isArray(rows) ? rows : []) {
    ...
    const fingerprint = safeString(row.text_fingerprint || row.textFingerprint || row.text_hash || row.textHash || row.source_fingerprint || "", 80);
    if (fingerprint && !group.sampleFingerprints.includes(fingerprint) && group.sampleFingerprints.length < 5) {
      group.sampleFingerprints.push(fingerprint);
    }
  ...
  const sorted = Array.from(groups.values()).filter((group) => group.sourceCount >= PUBLIC_MIN_NARRATIVE_SOURCES && group.eventCount >= PUBLIC_MIN_NARRATIVE_EVENTS).sort((a, b) => b.eventCount - a.eventCount).slice(0, Math.max(1, top)).map((group) => ({
    ...
    topKeywords: Array.from(group.hintCounts.entries()).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([label]) => label),
    sampleFingerprints: group.sampleFingerprints,
...
function normalizeTopicHints(rawJson) {
  ...
  return normalizeAmplificationTopicHints(Array.isArray(raw) ? raw : [], 6);
}
__name(normalizeTopicHints, "normalizeTopicHints");
function sanitizeHintLabel(raw) {
  return safeString(normalizeAmplificationTopicLabel(raw), 120);
}
...
function summarizeNarrativePattern(text, hints = []) {
  ...
  const title = anchors.length ? `\u4EE5 ${anchorText} \u70BA\u6838\u5FC3\u7684\u91CD\u8907\u6558\u4E8B` : "\u76F8\u8FD1\u63AA\u8FAD\u91CD\u8907\u6558\u4E8B";
...
function safeSignalScore(value) {
...
__name(safeSignalScore, "safeSignalScore");
...
async function ensurePlatformTables(env) {
  ...
  for (const col of [
    ...
    "ALTER TABLE platform_uploads ADD COLUMN upload_source TEXT DEFAULT 'extension'",
...
function normalizeTopicLabel(raw) {
  return safeString(normalizeAmplificationTopicLabel(raw), 140);
}
