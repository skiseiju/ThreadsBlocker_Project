...
| 8 | 2.6.4-beta1 | Fixed (2.6.4-beta1) | 大蟑螂回望提醒在小視窗/手機上仍可能因名單摘要太長而按不到確認或取消 | v2.6.1 雖限制最多列 30 個帳號，但 30 行在較矮 viewport 仍會壓縮 footer；通用 confirm 也缺少寬度、換行、安全區與按鈕換行保護 | `ui.js`：confirm modal 改為 viewport 內寬高、內容區強制捲動/換行、footer sticky 且支援 safe-area；`main.js`：回望摘要縮為 8 個帳號並保留「前 10 個」開啟上限 |
| 9 | 2.6.4-beta1 | Fixed (2.6.4-beta1) | 2.6.3 的平台上傳 raw 資料修復沒有合回主 repo，導致 duplicate 上傳仍可能只更新既有分析紀錄、不留下 raw 重傳資料 | 2.6.3 修正在 `ThreadsBlocker-analytics-upload` 工作樹完成並上傳 draft，但 main 工作樹仍停在 2.6.2 程式碼；server 端也以整份 raw JSON hash 當唯一鍵，duplicate 路徑直接 return | 合回 2.6.3 核心：新增 `platform_raw_ingests`；server 每次 ingest 先保存 raw payload，再以 canonical analysis hash 去重；client 升版後對已開啟自動上傳者跑一次 `repair_reupload_v1`，並延續舊版同意狀態 |
